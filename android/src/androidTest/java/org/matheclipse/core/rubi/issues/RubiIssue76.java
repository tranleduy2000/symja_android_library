...
public class RubiIssue76 extends AbstractRubiTestCase {
	...
	public void test0072() {
		check("Rubi`FreeFactors[-(F^a*(c + d*x)^2*Gamma[-2/3, -((b*Log[F])/(c + d*x)^3)]*(-((b*Log[F])/(c + d*x)^3))^(2/3))/(3*d), x]",
				"-F^a/(3*d)");
	}

	public void test0134() {
		check("Rubi`NonfreeFactors[-(F^a*(c + d*x)^2*Gamma[-2/3, -((b*Log[F])/(c + d*x)^3)]*(-((b*Log[F])/(c + d*x)^3))^(2/3))/(3*d), x]",
