...
public class Parser extends Scanner {
  ...
  private ASTNode getFactor(final int min_precedence) throws SyntaxError {
    ...
    switch (fToken) {
      ...
      case TT_PERCENT:
        ...
        final FunctionNode out = fFactory
            .createFunction(fFactory.createSymbol(IConstantOperators.Out));
      ...
      case TT_SLOT:
        ...
        getNextToken();
        final FunctionNode slot = fFactory
            .createFunction(fFactory.createSymbol(IConstantOperators.Slot));
        if (fToken == TT_DIGIT) {
      ...
      case TT_ASSOCIATION_OPEN:
        final FunctionNode function = fFactory
            .createFunction(fFactory.createSymbol(IConstantOperators.List));
        fRecursionDepth++;
        try {
          getNextToken();
    ...
    throwSyntaxError(
        "Error in factor at character: '" + fCurrentChar + "' (Token:" + fToken + " \\u"
  ...
  private ASTNode getPart(final int min_precedence) throws SyntaxError {
    ...
    do {
      if (function == null) {
        ...
        function = fFactory
            .createFunction(fFactory.createSymbol(IConstantOperators.Part), function);
      }
  ...
  private ASTNode parseInequality(final FunctionNode ast, final InfixOperator infixOperator) {
    ...
    final FunctionNode result = fFactory
        .createFunction(fFactory.createSymbol(IConstantOperators.Inequality));
