...
public class Trie<S, T> implements Map<S, T>, Serializable {
  ...
  public Trie(TrieSequencer<S> sequencer, T defaultValue) {
    this.root = new TrieNode<S, T>(null, defaultValue, null, 0, 0,
        new PerfectHashMap<TrieNode<S, T>>());
   ...
   */
  public T put(S query, final T value) {
    return this.put(query, new Function<T, T>() {
      @Override
      public T apply(T previousValue) {
        return value;
      }
    }, null);
  }
  ...
  private static class EmptyContainer<T> extends AbstractCollection<T> implements Set<T>,
      Iterator<T> {
