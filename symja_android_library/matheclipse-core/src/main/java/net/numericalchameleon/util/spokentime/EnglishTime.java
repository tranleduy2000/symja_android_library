...
public class EnglishTime extends SpokenTime implements NumberToWordsInterface, MilitaryTimeInterface {
    ...
    private final static String BLANK = " ", MIDNIGHT = "midnight", NOON = "noon", OCLOCK = "o'clock",
            A_QUARTER = "a quarter", // UK only
            QUARTER = "quarter", HALF = "half", AFTER = "after", // US only
            PAST = "past", // UK only
            TO = "to", MINUTES = "minutes", MINUTE = "minute", HUNDERT = "hundred";
    ...
    private void hoursInWords(int hours) {
        ...
        int h = hours;
        if (type == HourclockType.HOURCLOCK12_COLLOQUIAL_US || type == HourclockType.HOURCLOCK12_COLLOQUIAL_UK) {
            if (h >= 12) {
                h -= 12;
            }
            if (h == 0) {
    ...
    public void numberToWords(int number) {
        try {
            if (type == HourclockType.HOURCLOCK12_COLLOQUIAL_US || type == HourclockType.HOURCLOCK24_MILITARY_US) {
    ...
    public enum HourclockType {
        HOURCLOCK12_COLLOQUIAL_US, HOURCLOCK24_MILITARY_US, HOURCLOCK12_COLLOQUIAL_UK, HOURCLOCK24_MILITARY_UK
    }
}
