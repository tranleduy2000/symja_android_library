...
public final class FactorOutSimplifier implements FormulaTransformation {
    ...
    private static Formula computeMaxOccurringSubformula(final NAryOperator formula) {
        ...
        for (final Formula operand : formula) {
            if (operand.type() == FType.LITERAL) {
                MapWrapper.merge(formulaCounts, operand, 1, new BiFunction<Integer, Integer, Integer>() {
                    @Override
                ...
                for (final Formula subOperand : operand) {
                    MapWrapper.merge(formulaCounts, subOperand, 1, new BiFunction<Integer, Integer, Integer>() {
                        @Override
                        public Integer apply(Integer a, Integer b) {
                            return DInteger.sum(a, b);
                        }
        ...
        final Pair<Formula, Integer> max = new StreamWrapper<>(formulaCounts.entrySet()).stream()
                .max(Comparator.comparingInt(new ToIntFunction<Map.Entry<Formula, Integer>>() {
                    @Override
                    public int applyAsInt(Map.Entry<Formula, Integer> value) {
                        return value.getValue();
                    }
                }))
                .map(new Function<Map.Entry<Formula, Integer>, Pair<Formula, Integer>>() {
                    @Override
                    public Pair<Formula, Integer> apply(Map.Entry<Formula, Integer> e) {
                        return new Pair<>(e.getKey(), e.getValue());
                    }
                })
                .orElse(new Pair<Formula, Integer>(null, 0));
