...
public final class PlaistedGreenbaumTransformationSolver {
    ...
    private LNGIntVector handleImplication(final Implication formula, final boolean polarity, final Proposition proposition, final boolean topLevel) {
        ...
        final Pair<Boolean, Integer> pgVarResult = skipPg ? new Pair<Boolean, Integer>(false, null) : getPgVar(formula, polarity);
    ...
    private Pair<Boolean, Integer> getPgVar(final Formula formula, final boolean polarity) {
        final VarCacheEntry entry = new MapWrapper<>(this.variableCache).computeIfAbsent(formula, new Function<Formula, VarCacheEntry>() {
            @Override
            public VarCacheEntry apply(Formula i) {
                return new VarCacheEntry(PlaistedGreenbaumTransformationSolver.this.newSolverVariable());
            }
        });
