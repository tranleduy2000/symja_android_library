...
public final class OptimizationFunction implements SolverFunction<Assignment> {
    ...
    private Assignment maximize(final MiniSat solver) {
        ...
        if (this.maximize) {
            new MapWrapper<>(selectorMap).forEach(new BiConsumer<Variable, Literal>() {
                @Override
                public void accept(Variable selVar, Literal lit) {
                    solver.add(f.or(selVar.negate(), lit));
                }
            });
            new MapWrapper<>(selectorMap).forEach(new BiConsumer<Variable, Literal>() {
                @Override
                public void accept(Variable selVar, Literal lit) {
                    solver.add(f.or(lit.negate(), selVar));
                }
            });
        } else {
            new MapWrapper<>(selectorMap).forEach(new BiConsumer<Variable, Literal>() {
                @Override
                public void accept(Variable selVar, Literal lit) {
                    solver.add(f.or(selVar.negate(), lit.negate()));
                }
            });
            new MapWrapper<>(selectorMap).forEach(new BiConsumer<Variable, Literal>() {
                @Override
                public void accept(Variable selVar, Literal lit) {
                    solver.add(f.or(lit, selVar));
                }
            });
        }
