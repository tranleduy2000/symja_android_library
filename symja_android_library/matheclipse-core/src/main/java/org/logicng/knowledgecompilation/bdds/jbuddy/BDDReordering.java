...
public class BDDReordering {
    ...
    protected void reorderBlock(final BDDTree t, final BDDReorderingMethod method) {
        ...
        if (t.getSeq() != null) {
            ...
            t.setSeq(StreamWrapper.stream(t.getSeq())
                    .limit(t.getLast() - t.getFirst() + 1)
                    .boxed()
                    .sorted(new Comparator<Integer>() {
                        @Override
                        public int compare(Integer aa, Integer bb) {
                            return BDDReordering.this.varseqCmp(aa, bb);
                        }
                    })
    ...
    protected BDDTree reorderSift(BDDTree t) {
        ...
        Arrays.sort(p, 0, num, new Comparator<BDDSizePair>() {
            @Override
            public int compare(BDDSizePair a, BDDSizePair b) {
                return BDDReordering.this.siftTestCmp(a, b);
            }
        });
    ...
    protected int siftTestCmp(final BDDSizePair a, final BDDSizePair b) {
        return DInteger.compare(a.val, b.val);
    }
