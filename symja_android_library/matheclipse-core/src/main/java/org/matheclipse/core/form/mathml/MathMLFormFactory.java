...
public class MathMLFormFactory extends AbstractMathMLFormFactory {
    ...
    public boolean convert(final StringBuilder buf, final IAST f, final int precedence) {
      ...
      for (int i = size; i > 0; i--) {
        ...
        if ((i < size) && expr.isAST(F.Times)) {
          timesConverter
              .convertTimesFraction(buf, (IAST) expr, fPrecedence, MathMLFormFactory.PLUS_CALL);
        } else {
          if (i < size) {
  ...
  private void convertAST(final StringBuilder buf, final IAST ast, final int precedence) {
    ...
    if (!header.isSymbol()) {
      ...
      if (derivStruct != null) {
        ...
        if (a1Head.isAST1() && headAST.isAST1() && (headAST.arg1().isSymbol() || headAST.arg1()
            .isAST())) {
          try {
    ...
    final org.matheclipse.parser.client.operator.Operator operator = OutputFormFactory
        .getOperator(head);
    if (operator != null) {
  ...
  protected String convertDoubleToFormattedString(double dValue) {
    if (fSignificantFigures > 0) {
      try {
        StringBuilder buf = new StringBuilder();
        DoubleToMMA
            .doubleToMMA((Appendable) buf, dValue, fExponentFigures, fSignificantFigures, false);
        return buf.toString();
  ...
  public void convertInfixOperator(final StringBuilder buf, final IAST list,
    ...
    if (list.isAST2()) {
      ...
      if (oper.getGrouping() == InfixOperator.RIGHT_ASSOCIATIVE && list.arg1().head()
          .equals(list.head())) {
        // append(buf, "(");
        tag(buf, "mo", "(");
        // } else {
      ...
      if (oper.getGrouping() == InfixOperator.LEFT_ASSOCIATIVE && list.arg2().head()
          .equals(list.head())) {
        // append(buf, "(");
        tag(buf, "mo", "(");
      }
