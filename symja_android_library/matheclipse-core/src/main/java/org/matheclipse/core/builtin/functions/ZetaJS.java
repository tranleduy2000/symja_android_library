...
public class ZetaJS {
  ...
  public static Complex summation(
      Function<Complex, Complex> f, double a, double b, int iterationLimit) {
  ...
  public static Complex complexAverage(DoubleFunction<Complex> f, double x) {
    double offset = 1e-5;
  ...
  public static Complex complexSummation(
      DoubleFunction<Complex> f, double a, double b, int iterationLimit) {
    Complex s = Complex.ZERO;
  ...
  public static double sumDouble(
      DoubleUnaryOperator f, double a, double b, int iterationLimit) {
    double s = 0.0;
  ...
  public static double sumInt(
      IntToDoubleFunction f, int a, int b, int iterationLimit) {
    double s = 0;
  ...
  public static Complex hurwitzZeta(final Complex x, final Complex a) {
    ...
    if (a.getReal() < 0.0) {
      ...
      return hurwitzZeta(x, a.add(m))
          .add(summation(new Function<Complex, Complex>() {
            @Override
            public Complex apply(Complex i) {
              return a.add(i).pow(x.negate());
            }
          }, 0, m - 1.0, iterationLimit));
    }
    ...
    Complex S = summation(new Function<Complex, Complex>() {
      @Override
      public Complex apply(Complex i) {
        return a.add(i).pow(x.negate());
      }
  ...
  public static Complex polyLog(final Complex n, final Complex x) {
    ...
    while (Math.abs(p.getReal()) > Config.SPECIAL_FUNCTIONS_TOLERANCE
        || Math.abs(p.getImaginary()) > Config.SPECIAL_FUNCTIONS_TOLERANCE) {
      if (i++ > iterationLimit && iterationLimit > 0) {
        IterationLimitExceeded.throwIt(i, F.PolyLog);
      }
