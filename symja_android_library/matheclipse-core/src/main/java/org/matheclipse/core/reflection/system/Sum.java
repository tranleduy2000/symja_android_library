...
public class Sum extends ListFunctions.Table implements SumRules {
      ...
      if (argN.isList()) {
        ...
        if (argN.isList()) {
          ...
          if (argN.isReal()) {
            ...
            return IOFunctions
                .printMessage(ast.topHead(), "nliter", F.List(argN, F.ZZ(ast.size() - 1)),
                    engine);
          }
        }
   ...
   * @return
   */
  private IExpr definiteSum(final IExpr expr, final IIterator<IExpr> iterator, IAST list,
      EvalEngine engine) {
    ...
    if (expr.isFree(var, true)) {
      ...
      if (expr.isPower() && !engine.evalTrue(F.Greater(C1, from)) && !engine
          .evalTrue(F.Greater(from, to))) {
   ...
   * @return
   */
  private IExpr definiteSumInfinity(final IExpr expr, final IIterator<IExpr> iterator, IAST list,
      EvalEngine engine) {
    final ISymbol var = iterator.getVariable();
    final IExpr from = iterator.getLowerLimit();
    final IExpr to = iterator.getUpperLimit();
  ...
  public IExpr sumPowerFormula(IExpr from, final IExpr to, IInteger p) {
    ...
    if (!from.isOne()) {
      ...
      if (p.isOne()) {
        ...
        term1 = F.eval(
            ExpandAll(
                Plus(Times(Power(Plus(fromMinusOne, C1), Plus(p, C1)), Power(Plus(p, C1), CN1)),
                    Sum(
                        Times(Times(
                            Times(Power(Plus(fromMinusOne, C1), Plus(Plus(p, Times(CN1, k)), C1)),
    ...
    if (p.isOne()) {
      ...
      term2 = F.eval(ExpandAll(
          Plus(Times(Power(Plus(to, C1), Plus(p, C1)), Power(Plus(p, C1), CN1)), Sum(Times(
              Times(Times(Power(Plus(to, C1), Plus(Plus(p, Times(CN1, k)), C1)), Binomial(p, k)),
                  BernoulliB(k)),
