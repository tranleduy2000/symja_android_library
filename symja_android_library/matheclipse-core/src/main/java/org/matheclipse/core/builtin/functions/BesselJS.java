...
public class BesselJS {
  ...
  public static double besselJDouble(double n, double x) {
    ...
    return Math.pow(x / 2.0, n) * HypergeometricJS.hypergeometric0F1(n + 1, -0.25 * x * x) / GammaJS
        .gamma(n + 1);
  }
  ...
  public static double besselJZero(double n, int m) {
    ...
    ISymbol x = F.Dummy("x");
    UnivariateDifferentiableFunction f = new UnaryNumerical(F.BesselJ(F.num(n), x), x,
        EvalEngine.get(), true);
  ...
  public static double besselYDouble(double n, double x) {
    ...
    if (F.isNumIntValue(n)) {
      FiniteDifferencesDifferentiator differentiator = new FiniteDifferencesDifferentiator(15,
          0.01);
    ...
    return (besselJDouble(n, x) * Math.cos(n * Math.PI) - besselJDouble(-n, x)) / Math
        .sin(n * Math.PI);
  ...
  public static Complex besselY(Complex n, Complex x) {
    ...
    Complex sum = n.multiply(Math.PI).cos().multiply(besselJ(n, x))
        .subtract(besselJ(n.negate(), x));
...
//
    ...
    BisectionSolver solver = new BisectionSolver();
    ISymbol x = F.Dummy("x");
    UnivariateDifferentiableFunction f = new UnaryNumerical(F.BesselY(F.num(n), x), x,
        EvalEngine.get(), true);
    return solver.solve(100, f, e - delta, e + delta);
    // }
  }
  ...
  public static double struveL(double n, double x) {
    ...
    return
        Math.pow(x, n + 1.0) * 1 / (Math.pow(2.0, n) * Math.sqrt(Math.PI) * GammaJS.gamma(n + 1.5))
            * //
            HypergeometricJS.hypergeometric1F2(1, 1.5, n + 1.5, 0.25 * x * x);
  }
