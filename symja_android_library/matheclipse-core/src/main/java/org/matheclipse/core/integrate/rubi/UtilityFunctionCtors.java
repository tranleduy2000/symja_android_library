  ...
  public static IExpr evalRubiDistPlus(IAST astPlus, EvalEngine engine) {
    for (int i = 1; i < astPlus.size() - 1; i++) {
      ...
      if (arg1.isAST(Dist) && arg1.size() == 4) {
        // dist1 = Dist[u_,v_,x_]
        IAST dist1 = engine.evalArgs((IAST) arg1, ISymbol.NOATTRIBUTE)
            .orElse((IAST) arg1);// (IAST) arg1;
        IExpr v = dist1.arg2();
        IExpr x = dist1.arg3();
        for (int j = i + 1; j < astPlus.size(); j++) {
          IExpr arg2 = astPlus.get(j);
          if (arg2.isAST(Dist) && arg2.size() == 4 && arg2.getAt(2).equals(v) && arg2.getAt(3)
              .equals(x)) {
            // dist2=Dist[w_,v_,x_]
            IAST dist2 = engine.evalArgs((IAST) arg2, ISymbol.NOATTRIBUTE)
                .orElse((IAST) arg2);// (IAST)
            // arg2;
        ...
        // -1 * Dist[w_,v_,x_]
        IAST dist1 = engine.evalArgs((IAST) arg1.second(), ISymbol.NOATTRIBUTE)
