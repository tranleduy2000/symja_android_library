...
public final class Arithmetic {
   ...
   * </pre>
   */
  private static class Arg extends AbstractCoreFunctionEvaluator implements INumeric,
      DoubleUnaryOperator {
      ...
      try {
        if (ast.head().equals(S.Complex)) {
          ...
          if (realExpr instanceof INum && imaginaryExpr instanceof INum) {
            return F
                .complexNum(((INum) realExpr).doubleValue(), ((INum) imaginaryExpr).doubleValue());
          }
          if (realExpr.isNumber() && imaginaryExpr.isNumber()) {
    ...
    public IExpr evaluate(final IAST ast, EvalEngine engine) {
      ...
      try {
        if (symbol.isSymbol() && ((ISymbol) symbol).hasAssignedSymbolValue()) {
          IExpr[] results = ((ISymbol) symbol)
              .reassignSymbolValue(getAST(), getFunctionSymbol(), engine);
          if (results != null) {
        ...
        if (temp instanceof INum) {
          ...
          if (number instanceof Num) {
            ...
            complexNumber = complexNumber
                .add(F.complexNum(number.apfloatValue(number.precision())));
          }
        } else if (temp instanceof IComplexNum) {
...
//		}
    ...
    private static IExpr e2DblArg(final INum base, final INum exponent) {
      if (base.isZero()) {
        if (exponent.isNegative()) {
          IOFunctions
              .printMessage(F.Power, "infy", F.List(F.Power(F.C0, exponent)), EvalEngine.get());
        ...
        if (exponent.isZero()) {
          // 0^0
          IOFunctions
              .printMessage(F.Power, "indet", F.List(F.Power(F.C0, F.C0)), EvalEngine.get());
    ...
    private static IExpr fractionInteger(IFraction base, IInteger exponent) {
      ...
      if (exp.isNegative()) {
        ...
        return F
            .Rational(base.denominator().power(negExponent), base.numerator().power(negExponent));
      }
    ...
    private static IExpr e2ObjArg(IAST ast, final IExpr base, final IExpr exponent) {
      ...
      if (base.isReal() || exponent.isReal()) {
        ...
        if (exponent.isReal()) {
          ...
          if (base.isFraction() && base.isPositive() && ((IFraction) base).isLT(F.C1)) {
            IExpr o1negExpr = AbstractFunctionEvaluator
                .getPowerNegativeExpression(exponent, true);
            if (o1negExpr.isPresent()) {
      ...
      if (base.isAST()) {
        ...
        if (powBase.isTimes()) {
          ...
          if ((base.size() > 2)) {
            ...
            final IASTAppendable simplifiedTimesArgs = powBase.copyHead();
      ...
      if (exponent.isFraction() && base.isRational()) {
        ...
        if (((IFraction) exponent).isGT(F.C1)) {
          ...
          return F
              .Times(F.CN1, F.Power(F.CN1, F.C1.add(exponent)), F.Power(base.negate(), exponent));
        }
    ...
    private static IExpr evalDirectedInfinityArg2(final IExpr base, final IAST directedInfinity) {
      ...
      if (base.isOne() || base.isMinusOne() || base.isImaginaryUnit() || base
          .isNegativeImaginaryUnit()) {
        return F.Indeterminate;
      }
      if (base.isZero()) {
    ...
    private static IExpr powerZeroArg1(final IExpr exponent) {
      ...
      if (a.isNumericFunction(true)) {
        ...
        if (temp.isReal()) {
          if (temp.isNegative()) {
            IOFunctions
                .printMessage(F.Power, "infy", F.List(F.Power(F.C0, temp)), EvalEngine.get());
            // engine.printMessage("Infinite expression 0^(negative number)");
            return F.CComplexInfinity;
          }
          if (temp.isZero()) {
        ...
        if (temp.isComplex() || temp.isComplexNumeric()) {
          IOFunctions
              .printMessage(F.Power, "indet", F.List(F.Power(F.C0, temp)), EvalEngine.get());
    ...
    private static IAST powerEPlus(IAST plus) {
      ...
      for (int i = plus.argSize(); i > 0; i--) {
        ...
        if (temp.isLog()) {
          ...
          plusClone.remove(i);
        } else if (temp.isTimes() && temp.size() == 3 && temp.second().isLog() && temp.first()
            .isReal()) {
    ...
    public IExpr evaluate(final IAST ast, EvalEngine engine) {
      ...
      if (y.isPresent() && y.isRealResult()) {
        ...
        if (x.isPresent() && x.isRealResult()) {
          // (x + I*y)/Sqrt(x^2 + y^2)
          return F
              .Times(F.Plus(x, F.Times(F.CI, y)), F.Power(F.Plus(F.Sqr(x), F.Sqr(y)), F.CN1D2));
        }
      }
      if (arg1.isInterval()) {
        if (arg1.size() == 2) {
  ...
  private static class Surd extends AbstractArg2 implements INumeric {
    ...
    public IExpr evaluate(final IAST ast, EvalEngine engine) {
      ...
      if (arg2.isNumber()) {
        ...
        // Android changed: check numeric value is integer or not
        else if (arg2.isNumeric() && !arg2.isComplex() && arg2
            .isNumEqualInteger(((INum) arg2).integerPart())) {
          ...
          return IOFunctions
              .printMessage(ast.topHead(), "int", F.List(ast, F.C2), EvalEngine.get());
        }
      }
  ...
  public static class Times extends AbstractArgMultiple implements INumeric {
      ...
      for (int i = 2; i < times.size(); i++) {
        ...
        if (temp.isPlus()) {
          ...
          if (AbstractFunctionEvaluator.isNegativeWeighted(plus, true)) {
            temp = EvalEngine.get()
                .evaluate(plus.mapThread(F.binaryAST2(Times, CN1, F.Slot1), 2));
            result = times.copyAppendable();
            result.set(i, temp);
            result.remove(1);
            return result;
          ...
          case ID.Power:
            if (arg1.size() == 3) {
              ...
              if (arg2.isPower()) {
                ...
                IExpr temp = timesPowerPower(power0Base, power0Exponent, power1Base,
                    power1Exponent);
                if (temp.isPresent()) {
                  return temp;
                }
      ...
      for (int i = start; i < ast.size(); i++) {
        ...
        if (temp instanceof INum) {
          ...
          if (number instanceof Num) {
            ...
            complexNumber = complexNumber
                .multiply(F.complexNum(number.apfloatValue(number.precision())));
          }
        } else if (temp instanceof IComplexNum) {
          if (complexNumber instanceof ApcomplexNum) {
            complexNumber = complexNumber
    ...
    public IExpr evaluate(IAST ast1, EvalEngine engine) {
      ...
      if (size == 3) {
        ...
        IExpr temp = distributeLeadingFactor(
            binaryOperator(ast1, ast1.arg1(), ast1.arg2(), engine),
            ast1);
        if (!temp.isPresent()) {
  ...
  private static IExpr timesPowerPower(IExpr power0Arg1, IExpr power0Arg2, IExpr power1Arg1,
    ...
    if (power0Arg2.isNumber()) {
      if (power1Arg2.isNumber()) {
        ...
        if (power0Arg2.negate().equals(power1Arg2) && power0Arg1.isPositive() && power1Arg1
            .isPositive()
   ...
   * @return
   */
  public static IExpr timesPowerPower(IInteger p1Numer, IInteger p1Denom, IExpr p1Exp,
      IInteger p2Numer,
    ...
    IInteger fd2Rest = Primality
        .countPrimes1021(p2Denom, p2Exp.negate(), fn1Map, setEvaled, evaled);
    ...
    IInteger fd1Rest = Primality
        .countPrimes1021(p1Denom, p1Exp.negate(), fn2Map, setEvaled, evaled);
    if (!evaled[0]) {
  ...
  public static IAST piecewiseExpand(final IAST function, IBuiltInSymbol domain) {
    ...
    if (function.isAST(F.Clip, 2)) {
      IExpr x = function.arg1();
      return F
          .Piecewise(F.List(F.List(F.CN1, F.Less(x, F.CN1)), F.List(F.C1, F.Greater(x, F.C1))),
              x);
    }
    if (function.isAST(F.Clip, 3) && function.second().isAST(F.List, 3)) {
      ...
      return F
          .Piecewise(F.List(F.List(low, F.Less(x, low)), F.List(high, F.Greater(x, high))), x);
    }
