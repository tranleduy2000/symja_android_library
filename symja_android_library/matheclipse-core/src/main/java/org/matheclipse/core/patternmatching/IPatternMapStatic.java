...
public class IPatternMapStatic {
   ...
   * @param pattern
   */
  public static void addPattern(List<Pair<IExpr, IPatternObject>> patternIndexMap,
  ...
  static int determinePatternsRecursive(
      final List<Pair<IExpr, IPatternObject>> patternIndexMap,
      final IAST lhsPatternExpr,
      final int[] priority,
      final boolean[] ruleWithoutPattern,
      final int treeLevel) {
    ...
    for (int i = 0; i < lhsPatternExpr.size(); i++) {
      IExpr x = lhsPatternExpr.get(i);
      {
        if (x.isASTOrAssociation()) {
          ...
          listEvalFlags/*[0]*/ |= determinePatternsRecursive(patternIndexMap, lhsPatternAST,
              priority,
