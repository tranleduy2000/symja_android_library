      ...
      if (leftHandSide.head().isSymbol()) {
        ...
        if (temp == null) {
          ...
          if (symbol.isProtected()) {
            ...
            return IOFunctions
                .printMessage(builtinSymbol, "wrsym", F.List(symbol), EvalEngine.get());
          }
          try {
      ...
      if (size == 3) {
        try {
          ...
          if (arg1.isListOfRulesOrAssociation(false)) {
            ...
            return keySelect(list, new Predicate<IExpr>() {
              @Override
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      try {
        if (ast.arg1().isListOfRulesOrAssociation(true) || ast.arg1().isListOfLists()) {
          ...
          return engine
              .printMessage("KeyTake: Association or list of rules expected at position 1.");
        }
      } catch (final ValidateException ve) {
        return engine.printMessage(ast.topHead(), ve);
