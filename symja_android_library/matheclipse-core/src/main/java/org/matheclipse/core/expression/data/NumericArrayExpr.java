...
import com.gx.common.primitives.UnsignedInts;
  ...
  private static void normalRecursive(
    ...
    if (dimension.length - 1 == position) {
      for (int i = 1; i <= size; i++) {
        IInteger intValue =
            unsigned
                ? F.ZZ(DByte.toUnsignedInt(byteArray[index[0]++]))
  ...
  private static void normalRecursive(
    ...
    if (dimension.length - 1 == position) {
      for (int i = 1; i <= size; i++) {
        IInteger intValue =
            unsigned
                ? F.ZZ(DShort.toUnsignedInt(shortArray[index[0]++]))
  ...
  private static void normalRecursive(
    ...
    if (dimension.length - 1 == position) {
      for (int i = 1; i <= size; i++) {
        long value =
            unsigned
                ? DInteger.toUnsignedLong(intArray[index[0]++]) //
                : intArray[index[0]++];
