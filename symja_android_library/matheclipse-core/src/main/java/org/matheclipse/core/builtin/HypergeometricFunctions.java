...
public class HypergeometricFunctions {
  ...
  private static class CoshIntegral extends
      AbstractFunctionEvaluator {// implements INumeric, DoubleUnaryOperator {
  ...
  private static class ExpIntegralEi extends
      AbstractFunctionEvaluator {// implements INumeric, DoubleUnaryOperator {
    // @Override
    // public double applyAsDouble(double operand) {
    // if (F.isZero(operand)) {
          ...
          return F.sum(
              new Function<IExpr, IExpr>() {
                ...
                public IExpr apply(IExpr k) {
                  return F.Times(F.Power(F.CN1, k),
  ...
  private static class Hypergeometric0F1 extends AbstractFunctionEvaluator implements
      ...
      try {
        ...
        if (a.isNumEqualInteger(F.C2)) {
          ...
          return F.Times(F.Plus(F.CN1, b), F.Plus(F.C1,
              F.Times(F.Plus(F.C2, F.Negate(b)), F.Power(F.E, z),
  ...
  private static class Hypergeometric2F1 extends AbstractFunctionEvaluator implements
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      ...
      try {
        ...
        if (engine.isDoubleMode()) {
          ...
          if (Double.isNaN(aDouble) || Double.isNaN(bDouble) || Double.isNaN(cDouble) || Double
              .isNaN(zDouble)
  ...
  private static class HypergeometricU extends AbstractFunctionEvaluator implements
      ...
      try {
                ...
                F.intSum( //
                    new IntFunction<IExpr>() {
                      ...
                      public IExpr apply(int k) {
                        return F.Times(F.Binomial(nMinus1, nMinus1 - k), F.Pochhammer(a, F.ZZ(k)),
                            F.Power(z, -k));
                      }
                    }, //
                    0, nMinus1));
        ...
        if (engine.isDoubleMode()) {
          ...
          try {
            ...
            zDouble = z.evalDouble();
            return F.complexNum(
                HypergeometricJS.hypergeometricU(new Complex(aDouble), new Complex(bDouble),
                    new Complex(zDouble)));
          } catch (ValidateException ve) {
            if (FEConfig.SHOW_STACKTRACE) {
              ve.printStackTrace();
  ...
  private static class LogIntegral extends
      AbstractFunctionEvaluator {// implements INumeric, DoubleUnaryOperator {
  ...
  private static class SinIntegral extends
      AbstractFunctionEvaluator {// implements INumeric, DoubleUnaryOperator {
      ...
      if (engine.isDoubleMode()) {
        try {
          ...
          try {
            ...
            zDouble = z.evalDouble();
            return F
                .complexNum(HypergeometricJS.whittakerM(new Complex(kDouble), new Complex(mDouble),
                    new Complex(zDouble)));
          } catch (ValidateException ve) {
            if (FEConfig.SHOW_STACKTRACE) {
              ve.printStackTrace();
