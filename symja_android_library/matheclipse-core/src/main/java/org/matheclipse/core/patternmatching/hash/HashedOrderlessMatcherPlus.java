  ...
  protected boolean updateHashValues(IASTAppendable result, final IAST orderlessAST,
    ...
    if ((temp = hashRule.evalDownRule(arg1, null, arg2, null, engine)).isPresent()) {
      ...
      if (intFactor1.isPositive() && intFactor2.isPositive()) {
        ...
        if (diff.isPositive()) {
          // num1 > num2
          result.append(F.Times(plusMinusOne, F.Plus(F.Times(diff, arg1), F.Times(intFactor2, temp))));
          return true;
        } else {
          // num1 < num2
          diff = diff.negate();
          result.append(F.Times(plusMinusOne, F.Plus(F.Times(diff, arg2), F.Times(intFactor1, temp))));
          return true;
        }
      }
      }
