...
public class SimplifyFunctions {
  ...
  private static class Simplify extends AbstractFunctionEvaluator {
    ...
    private static class SimplifyVisitor extends VisitorExpr {
      ...
      private IExpr tryTransformations(IExpr expr) {
        ...
        if (expr.isTimes()) {
          ...
          IExpr[] commonFactors = Algebra.InternalFindCommonFactorPlus
              .findCommonFactors((IAST) expr, true);
        ...
        if (powerAST.isPowerReciprocal() && powerAST.base().isPlus()
          ...
          for (int i = 1; i < plusAST.size(); i++) {
            ...
            if (plusArg.isTimes()) {
              ...
              int indx1 = timesAST.indexOf(new Predicate<IExpr>() {
                @Override
                public boolean test(IExpr x) {
                  return x.isLog();
                }
              ...
              if (indx1 > 0) {
                int indx2 = timesAST.indexOf(new Predicate<IExpr>() {
                  @Override
                  public boolean test(IExpr x) {
                    return x.isLog();
                  }
      ...
      private IExpr visitTimes(IASTMutable timesAST, SimplifiedResult sResult) {
        ...
        while (i < timesAST.size()) {
          ...
          if (timesArg.isPlus()) {
            IExpr negExpr = AbstractFunctionEvaluator
                .getNormalizedNegativeExpression(timesArg.first());
      ...
      private IExpr visitPlus(IASTMutable plusAST, SimplifiedResult sResult) {
        ...
        final IASTAppendable basicPlus = F.PlusAlloc(plusAST.size());
        final IASTAppendable restPlus = F.PlusAlloc(plusAST.size());
        ...
        if (timesAST.size() > 2 && timesAST.first().isInteger() && !timesAST.first().isMinusOne()) {
          ...
          for (int i = 2; i < timesAST.size(); i++) {
            ...
            if (temp.isLog() && temp.first().isReal()) {
              IAST result = timesAST
                  .splice(i, 1, F.Log(F.Power.of(temp.first(), timesAST.first())));
      ...
      private void functionExpand(IExpr expr,
          SimplifiedResult sResult) {// long minCounter, IExpr result) {
        if (fFullSimplify) {
          try {
    ...
    public IExpr evaluate(final IAST ast, EvalEngine engine) {
      ...
      try {
        Function<IExpr, Long> complexityFunction = createComplexityFunction(complexityFunctionHead,
            engine);
