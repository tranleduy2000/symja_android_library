...
import org.matheclipse.core.visit.VisitorRemoveLevelSpecification.StopException;
...
public final class ListFunctions {
      ...
      if (arg1.isASTOrAssociation()) {
        ...
        return foldLeft(null, list, 1, size, new BiFunction<IExpr, IExpr, IExpr>() {
          ...
          public IExpr apply(IExpr x, IExpr y) {
            return F.binaryAST2(S.Plus, x, y);
          }
        }, resultList);
  ...
  private static final class ArrayPad extends AbstractFunctionEvaluator {
    ...
    private static IExpr arrayPadMatrixAtom(final IAST matrix, int[] dim, final int m, final int n,
        final IExpr atom) {
  ...
  private static final class Cases extends AbstractCoreFunctionEvaluator {
    ...
    private static Consumer<? super IExpr> appendIfMatched(
        final IPatternMatcher matcher, final IASTAppendable resultAST) {
      return new Consumer<IExpr>() {
        @Override
  ...
  private static final class Catenate extends AbstractEvaluator {
    ...
    public IExpr evaluate(final IAST ast, EvalEngine engine) {
      ...
      if (ast.arg1().isList()) {
        ...
        if (list.forAll(isListOrAssociation(calculatedAllocSize))) {
          final IASTAppendable resultList = F.ast(S.List, calculatedAllocSize[0], false);
     ...
     * @return
     */
    private static Predicate<? super IExpr> isListOrAssociation(final int[] calculatedAllocSize) {
    ...
    public static IExpr evaluateComposeList(final IAST ast, final IASTAppendable resultList) {
      try {
        if ((ast.isAST2()) && (ast.arg1().isAST())) {
          ...
          ListFunctions
              .foldLeft(ast.arg2(), list, 1, list.size(), new BiFunction<IExpr, IExpr, IExpr>() {
                @Override
                public IExpr apply(IExpr x, IExpr y) {
  ...
  private static class Delete extends AbstractCoreFunctionEvaluator {
    ...
    private IAST deleteListOfPositions(final IAST list, final IAST listOfIntPositions,
      ...
      try {
        indx = Validate
            .checkListOfInts(list, listOfIntPositions, Integer.MIN_VALUE, Integer.MAX_VALUE,
                engine);
        if (indx == null) {
          return F.NIL;
        }
  ...
  private static final class FirstCase extends AbstractFunctionOptionEvaluator {
    ...
    private static IExpr firstCase(
        ...
        EvalEngine engine) {
      if (pattern.isRuleAST()) {
        final Function<IExpr, IExpr> function = Functors.rules((IAST) pattern, engine);
        final IExpr[] result = new IExpr[]{F.NIL};
        int index = list.indexOf(new Predicate<IExpr>() {
          @Override
          public boolean test(IExpr x) {
            return ruleEval(x, function, result);
          }
        });
    ...
    public IExpr evaluate(final IAST ast, EvalEngine engine) {
      ...
      if (ast.isAST3()) {
        if (ast.arg3().isZero()) {
          ...
          return IOFunctions
              .printMessage(ast.topHead(), "infy", F.List(F.Divide(ast.arg2(), F.C0)), engine);
        }
  ...
  private static final class Replace extends AbstractEvaluator {
    ...
    private static IExpr replaceExprWithLevelSpecification(final IAST ast, IExpr arg1, IExpr rules,
      ...
      VisitorLevelSpecification level = new VisitorLevelSpecification(replaceFunction,
          exprLevelSpecification,
          false, engine);
  ...
  private static final class ReplacePart extends AbstractEvaluator {
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      try {
        IExpr_COMPARE_TERNARY heads = IExpr_COMPARE_TERNARY.UNDECIDABLE;
        if (ast.size() > 3) {
          final OptionArgs options = new OptionArgs(ast.topHead(), ast, 3, engine);
          IExpr option = options.getOption(S.Heads);

          if (option.isTrue()) {
            heads = IExpr_COMPARE_TERNARY.TRUE;
          } else if (option.isFalse()) {
            heads = IExpr_COMPARE_TERNARY.FALSE;
          } else {
    ...
    public IExpr evaluate(final IAST ast, EvalEngine engine) {
      ...
      if (ast.size() == 3) {
        ...
        IAST factorList = Range.range(0, n + 1).map(new Function<IExpr, IExpr>() {
          @Override
          public IExpr apply(IExpr x) {
            return x.divide(arg2);
          }
        }, 1);
        return factorList.map(new Function<IExpr, IExpr>() {
          @Override
          public IExpr apply(IExpr x) {
            return arg1.times(x);
          }
      ...
      IAST factorList = Range.range(0, n + 1).map(new Function<IExpr, IExpr>() {
        @Override
        public IExpr apply(IExpr x) {
          return x.divide(arg3);
        }
      }, 1);
      return factorList.map(new Function<IExpr, IExpr>() {
        @Override
        public IExpr apply(IExpr x) {
          return arg1.plus(arg2.times(x).subtract(arg1.times(x)));
        }
      }, 1);
    }
    ...
    private static IAST take(final IAST list, final int level,
      ...
      if (step < 0) {
        ...
        if (start < end || end <= 0 || start >= list.size()) {
          ...
          String str = IOFunctions
              .getMessage("take", F.List(F.ZZ(start), F.ZZ(end), list), EvalEngine.get());
        ...
        for (int i = start; i < end; i += step) {
          ...
          if (sequenceSpecifications.length > newLevel) {
            if (arg.isAssociation()) {
              ...
              // List expected at position `1` in `2`.
              String str = IOFunctions
                  .getMessage("list", F.List(F.ZZ(i), assoc2), EvalEngine.get());
              throw new ArgumentTypeException(str);
            }
          } else {
            resultAssoc.appendRule(rule);
  ...
  private static IAST cleanList(IAST list) {
    return list.select(
        new Predicate<IExpr>() {
          @Override
          public boolean test(IExpr x) {
            return !(x.equals(S.Indeterminate)
                || x.equals(S.Null)
                || x.equals(S.None)
                || x.isAST(S.Missing));
          }
