  ...
  private static final class RandomChoice extends AbstractFunctionEvaluator {
    ...
    public IExpr evaluate(final IAST ast, EvalEngine engine) {
      ...
      if (arg1.isRuleAST()
        ...
        final IAST items = (IAST) arg1.second();
        double[] itemWeights = weights.toDoubleVector();
        if (itemWeights != null) {
          final EnumeratedDistributionSampler sampler = new EnumeratedDistributionSampler(itemWeights);
          if (ast.size() == 2) {
        ...
        if (ast.isAST2()) {
          ...
          if (arg2.isList()) {
            ...
            final int[] randomValue = new int[1];
            return ArrayBuilder.build(
                new Supplier<IExpr>() {
                  @Override
                  public IExpr get() {
