...
public class MinMaxFunctions {
  ...
  private static class NMinimize extends AbstractFunctionEvaluator {
    ...
    protected static List<LinearConstraint> getConstraints(final VariablesSet vars,
        IAST listOfconstraints) {
      final List<LinearConstraint> constraints = new ArrayList<LinearConstraint>(
          listOfconstraints.size());
  ...
  private static IExpr maximize(ISymbol head, IExpr function, IExpr x, EvalEngine engine) {
    ...
    return engine
        .printMessage(head.toString() + ": only unary functions in " + x + " are supported.");
  }
