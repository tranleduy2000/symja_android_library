...
/**
 * Ported from JavaScript file <a href="https://github.com/paulmasson/math/blob/master/src/functions/gamma.js">gamma.js</a>
 */
public class GammaJS {
                  ...
                  ZetaJS.complexSummation(
                      new DoubleFunction<Complex>() {
                        ...
                        public Complex apply(double k) {
                          return new Complex(Math.pow(-1.0, k) * factorialInt(k))
                              .divide(y.pow(k + 1));
                        }
                      }, //
                      0.0,
  ...
  public static Complex sinIntegral(Complex x) {
    ...
    Complex result = new Complex(0, 0.5)
        .multiply(gamma(Complex.ZERO, ix.negate()).add(gammaZero(ix).negate())
            .add(ix.negate().log()).add(ix.log().negate()));
  ...
  public static Complex cosIntegral(Complex x) {
    ...
    Complex result = x.log()
        .subtract(gammaZero(ix.negate()).add(gammaZero(ix)).add(ix.negate().log()).add(ix.log())
            .multiply(0.5));
    return result;
  }
