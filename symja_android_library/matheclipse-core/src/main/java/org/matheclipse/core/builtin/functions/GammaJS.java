...
public class GammaJS {
	...
	public static Complex sinIntegral(Complex x) {
		...
		Complex result = new Complex(0, 0.5).multiply(gamma(Complex.ZERO, ix.negate()).add(gammaZero(ix).negate())
	...
	public static Complex cosIntegral(Complex x) {
		...
		Complex result = x.log()
				.subtract(gammaZero(ix.negate()).add(gammaZero(ix)).add(ix.negate().log()).add(ix.log()).multiply(0.5));
		return result;
	}
	...
	public static Complex sinhIntegral(Complex x) {
		...
		Complex result = gammaZero(x).add(gammaZero(x.negate()).negate()).add(x.log()).add(x.negate().log().negate())
				.multiply(0.5);
