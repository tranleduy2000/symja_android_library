...
/**
 * Ported from JavaScript file <a href= "https://github.com/paulmasson/math/blob/master/src/functions/elliptic-functions.js">elliptic-functions.js</a>
 */
public class EllipticFunctionsJS {
  ...
  private static Complex cubicTrigSolution(Complex p, Complex q, int n) {
    ...
    return p.sqrt()
        .multiply(q.multiply(p.pow(-1.5)).multiply(3.0 * Math.sqrt(3.0) / 2.0).acos().divide(3.0)
  ...
  public static Complex[] weierstrassInvariants(Complex w1, Complex w3) {
    ...
    Complex g2 = w1.multiply(2).pow(-4)
    ...
    Complex g3 = w1.multiply(2.0).pow(-6.0).multiply(8.0 / 27.0 * Math.pow(Math.PI, 6.0))
        .multiply(a.pow(12)
            .add(aPow8.multiply(bPow4).multiply(-1.5)).add(aPow4.multiply(bPow8).multiply(-1.5))
            .add(b.pow(12)));
