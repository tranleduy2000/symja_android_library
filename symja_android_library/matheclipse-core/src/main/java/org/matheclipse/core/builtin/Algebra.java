...
public class Algebra {
      ...
      if (list.size() > 2) {
        ...
        if (map.size() != 0) {
          ...
          if (!result[0].isOne()) {
            ...
            final IASTAppendable commonPlus = F.PlusAlloc(list.size());
      ...
      if (exponent.isReal()) {
        ...
        IAST[] result = plusAST.filterNIL(new Function<IExpr, IExpr>() {
          ...
          public IExpr apply(IExpr expression) {
            return AbstractFunctionEvaluator.getNormalizedNegativeExpression(expression);
          }
        });
  ...
  private static class Cancel extends AbstractFunctionEvaluator {
    ...
    /**
     * This predicate identifies polynomial expressions. It requires that the given expression is
     * already expanded for <code>Plus,Power and Times</code> operations.
     */
     ...
     */
    private static IExpr[] calculatePlusIntegerGCD(final IASTAppendable numeratorPlus,
        IInteger denominatorInt,
        final IInteger gcd) {
    ...
    public static IExpr togetherPowerTimes(IExpr powerTimesAST) throws JASConversionException {
      ...
      if (parts != null && parts[0].isPlus() && parts[1].isPlus()) {
        IAST numParts = ((IAST) parts[0])
            .partitionPlus(new PolynomialPredicate(), F.C0, F.C1, F.List);
        IAST denParts = ((IAST) parts[1])
            .partitionPlus(new PolynomialPredicate(), F.C0, F.C1, F.List);
  ...
  private static class Collect extends AbstractCoreFunctionEvaluator {
    ...
    public void collectToMap(IExpr expr, final IPatternMatcher matcher,
        final Map<IExpr, IASTAppendable> map,
        IASTAppendable rest) {
      if (expr.isFree(matcher, false)) {
        ...
        if (timesAST.exists(new ObjIntPredicate<IExpr>() {
          @Override
    ...
    public boolean collectToMapPlus(IExpr expr, final IPatternMatcher matcher,
        final Map<IExpr, IASTAppendable> map) {
      if (expr.isFree(matcher, false)) {
        return false;
      } else if (matcher.test(expr)) {
   ...
   * </pre>
   */
  private static final class Distribute extends AbstractCoreFunctionEvaluator {
    ...
    public IExpr evaluate(final IAST ast, EvalEngine engine) {
      ...
      if (arg1.isAST()) {
        ...
        distributePosition(resultCollector, stepResult, head, list, 1);
    ...
    private void distributePosition(final IASTAppendable resultCollector,
        final IASTAppendable stepResult, final IExpr head,
        final IAST arg1, final int position) {
  ...
  private static class Expand extends AbstractFunctionEvaluator {
    ...
    private static class Expander {
      ...
      private IExpr expandAST(final IAST ast, boolean evalParts) {
        ...
        if (ast.isPower()) {
          ...
          if (distributePlus && temp[0].isPlus()) {
            IAST mappedAST = ((IAST) temp[0])
                .mapThreadEvaled(EvalEngine.get(), F.Times(null, powerAST), 1);
            IExpr flattened = flattenOneIdentity(mappedAST,
                F.C0);// EvalAttributes.flatten(mappedAST).orElse(mappedAST);
      ...
      public IExpr expandPlus(final IAST ast) {
        ...
        for (int i = 1; i < ast.size(); i++) {
          ...
          result.ifAppendable(new Consumer<IASTAppendable>() {
            @Override
    ...
    private static class NumberPartititon {
      ...
      private void addFactor(int[] j) {
        final Permutations.KPermutationsIterable perm = new Permutations.KPermutationsIterable(j, m,
            m);
                ...
                if (temp.isTimes()) {
                  ...
                  timesAST.appendArgs(ast.size(), new IntFunction<IExpr>() {
                    ...
                    public IExpr apply(int i) {
                      return F.Power(ast.get(i), F.ZZ(indices[ki]));
                    }
                  });
                } else {
  ...
  private static class Factor extends AbstractFunctionEvaluator {
    ...
    public void setUp(final ISymbol newSymbol) {
      setOptions(
          newSymbol, //
          F.List(F.Rule(F.GaussianIntegers, F.False), F.Rule(F.Modulus, F.C0)));
    }
    ...
    public IExpr factorExpr(final IAST ast, IExpr expr, final VariablesSet eVar,
        final boolean factorSquareFree,
        final EvalEngine engine) {
      if (expr.isAST()) {
        IExpr temp;
    ...
    private static IExpr factorWithPolynomialHomogenization(IAST expr, VariablesSet eVar,
        EvalEngine engine) {
      boolean gaussianIntegers = !expr.isFree(new Predicate<IExpr>() {
        @Override
        public boolean test(IExpr x) {
          return x.isComplex() || x.isComplexNumeric();
        }
      }, false);
      ...
      if (subsPolynomial.isAST()) {
        ...
        IExpr factorization = factorComplex(subsPolynomial, eVar.getArrayList(), F.Times,
            gaussianIntegers,
            engine);
    ...
    public IExpr evaluate(final IAST ast, EvalEngine engine) {
      ...
      if (checkPolyStruct(ast, engine)) {
        ...
        try {
          ...
          GreatestCommonDivisorAbstract<BigInteger> factory = GCDFactory
              .getImplementation(BigInteger.ZERO);
   ...
   * </blockquote>
   */
  private static class PolynomialQ extends AbstractCoreFunctionEvaluator implements
      BiPredicate<IExpr, IExpr> {
    ...
    public IExpr evaluate(final IAST ast, EvalEngine engine) {
      ...
      try {
        ...
        if (ast.size() == 5) {
          ...
          IExpr option = options.getOption(F.Modulus);
          if (option.isInteger() && !option.isZero()) {
            IExpr[] quotientRemainderModInteger = quotientRemainderModInteger(arg1, arg2, variable,
                option);
  ...
  private static class PowerExpand extends AbstractFunctionEvaluator {
    ...
    private static class PowerExpandVisitor extends VisitorExpr {
      ...
      public IExpr visit2(IExpr head, IExpr arg1) {
        ...
        } else if (head.equals(F.ProductLog)) {
          if (arg1.isTimes2()) {
          ...
          if (x1.isTimes()) {
            ...
            if (assumptions) {
              ...
              plusResult.appendArgs(
                  timesAST.size(), new IntFunction<IExpr>() {
                    @Override
                    public IExpr apply(int i) {
    ...
    public void setUp(final ISymbol newSymbol) {
      ...
      setOptions(
          newSymbol, //
          F.List(F.Rule(F.Assumptions, F.Automatic)));
    }
  }
  ...
  private static class ToRadicals extends AbstractFunctionEvaluator {
    ...
    private static IExpr root2(IExpr a, IExpr b, IExpr c, int nthRoot) {
      ...
      return Plus(Times(C1D2, Power(F.CN1, k),
    ...
    private static IExpr root3(IExpr a, IExpr b, IExpr c, IExpr d, int nthRoot) {
      ...
      IExpr q = Plus(Times(F.CN2, Power(c, 3)), Times(F.C9, b, c, d),
          Times(F.ZZ(-27), a, F.Sqr(d)));
      ...
      return Plus(Times(F.CN1D3, c, Power(d, -1)),
          Times(F.CN1D3, Power(F.E, Times(F.CC(0L, 1L, -2L, 3L), Plus(F.CN1, k), F.Pi)),
              Power(p, -1), r,
              Power(C2, F.C1D3), Power(d, -1)),
          Times(F.C1D3, Power(C2, F.CN1D3),
              Power(F.E, Times(F.CC(0L, 1L, 2L, 3L), Plus(F.CN1, k), F.Pi)),
              Power(d, -1), p));
    }
  ...
  private static class Together extends AbstractFunctionEvaluator {
    ...
    private static IBuiltInSymbol reduceConstantTerm = F
        .localFunction("reduceConstantTerm", new Function<IExpr, IExpr>() {
          @Override
          public IExpr apply(IExpr c) {
            if (c.isNumber()) {
    ...
    private static IExpr reduceFactorConstant(IExpr p, EvalEngine engine) {
      if (!engine.isNumericMode() && p.isPlus() && !engine.isTogetherMode()) {
        ...
        IExpr cTerms = F.Transpose
            .of(engine,
                F.Map(F.Function(F.unaryAST1(reduceConstantTerm, F.Slot1)), F.Apply(F.List, e)))
            .first();
        // GCD @@ cTerms
        ...
        IExpr gcd;
        if (!c.isFree(Predicates.isInexactNumber, false)) {
          ...
          if (!arg1.exponent().isMinusOne()) {
            if (arg1.base().isPlusTimesPower()) {
              if (arg1.exponent().isNegative()) {
                return F
                    .Power(togetherExpr(arg1.base().inverse(), engine), arg1.exponent().negate());
              }
   ...
   */
  public static IExpr expand(final IAST ast, IExpr patt, boolean expandNegativePowers,
      boolean distributePlus,
      boolean evalParts) {
   ...
   * @return <code>F.NIL</code> if the expression couldn't be expanded.
   */
  public static IExpr expandAll(final IAST ast, final IExpr patt,
      final boolean expandNegativePowers, final boolean distributePlus,
      final EvalEngine engine) {
    ...
    if (head.isAST()) {
      ...
      temp.ifPresent(new Function<IExpr, IExpr>() {
        @Override
        public IExpr apply(IExpr x) {
          return result[0] = F.ast(x, localASTSize, false);
        }
      });
    }
  ...
  private static IExpr factorComplex(IExpr expr, GenPolynomial<Complex<BigRational>> polynomial,
    ...
    for (SortedMap.Entry<GenPolynomial<Complex<BigRational>>, Long> entry : map.entrySet()) {
      ...
      if (entry.getValue().equals(1L) && map.size() <= 2 && (key.equals(F.CNI) || key
          .equals(F.CI))) {
  ...
  public static IExpr[] fractionalPartsTimesPower(final IAST timesPower, boolean splitNumeratorOne,
    ...
    if (evaled) {
      ...
      if (negateNumerDenom && result[0].isNegative() && result[1].isPlus() && ((IAST) result[1])
          .isAST2()) {
    ...
    if (splitFractionEvaled) {
      ...
      if (result[0].isTimes() && ((IAST) result[0]).isAST2() && ((IAST) result[0]).arg1()
          .isMinusOne()) {
        result[1] = denominator.oneIdentity1();
        return result;
      }
   ...
   * @param denominatorTimes the <codeTimes( ... )</code> expression of the denominator of the fraction expression
   * @param variable
  ...
  public static IExpr partialFractionDecomposition(IExpr numerator, IExpr denominatorTimes,
    ...
    if (first.isFree(variable)) {
      ...
      if (peGCD.isList() && peGCD.second().isList()) {
        ...
        IExpr u1 = F.PolynomialRemainder
            .ofNIL(engine, F.Expand(F.Times(B, numerator)), v1, variable);
        if (u1.isPresent()) {
          IExpr u2 = F.PolynomialRemainder
              .ofNIL(engine, F.Expand(F.Times(A, numerator)), v2, variable);
          if (u2.isPresent()) {
            return F.Plus.of(engine, F.Times(u1, F.Power(first, -1)),
                partialFractionDecomposition(u2, rest, variable, count + 1, engine));
          }
