...
public class StatisticsFunctions {
	...
	private final static class Covariance extends AbstractMatrix1Expr {
		...
		public static IExpr vectorCovarianceSymbolic(final IAST arg1, final IAST arg2, int arg1Length) {
			...
			v1.appendArgs(arg1.size(), new IntFunction<IExpr>() {
				@Override
	...
	private final static class MeanDeviation extends AbstractFunctionEvaluator {
		...
		public IExpr evaluate(final IAST ast, EvalEngine engine) {
			...
			if (length > 0) {
				...
				// x -> sum.append(F.Abs(F.Plus(x, mean)))
				);
							...
							if (vector.forAll(new Predicate<IExpr>() {
								...
								return vector.map(new Function<IExpr, IExpr>() {
									...
									public IExpr apply(IExpr scalar) {
										return of(s, length, (ISignedNumber) scalar);
									}
								},
										// scalar -> of(s, length, (ISignedNumber) scalar),
										1);
							}
						} else {
							if (q.isSignedNumber()) {
						...
						if (evaluator instanceof IRandomVariate) {
							...
							if (ast.size() == 3) {
								...
								if (arg2.isList()) {
									...
									return createArray(indx, 0, list, 
											new Supplier<IExpr>(){
												@Override
												public IExpr get() {
