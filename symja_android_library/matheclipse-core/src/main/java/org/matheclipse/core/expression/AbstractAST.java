...
public abstract class AbstractAST extends IASTMutableImpl {
  ...
  private final static class NILPointer extends AbstractAST implements INilPointer, IAssociation {
    ...
    /** {@inheritDoc} */
    @Override
    public final String internalJavaString(boolean symbolsAsFactoryMethod, int depth,
  ...
  @Override
  public IExpr_COMPARE_TERNARY equalTernary(IExpr that, EvalEngine engine) {
          ...
          new ObjIntConsumer<IExpr>() {
            ...
            public void accept(IExpr x, int i) {
              if (x.isAST(S.Evaluate)) {
                ...
                engine.evalArg(rlist, AbstractAST.this, temp, x, i, false);
              }
            }
          });
    }
  ...
  /** {@inheritDoc} */
  @Override
  public IAST filter(final IASTAppendable filterAST, final IASTAppendable restAST,
      final Predicate<? super IExpr> predicate) {
  ...
  public String internalJavaString(
    ...
    if (useOperators && size() == 3) {
      if (isTimes()) {
        ...
        internalOperatorForm(arg1, isLowerPrecedence, symbolsAsFactoryMethod, depth, useOperators,
            usePrefix,
            noSymbolPrefix, text);
        text.append('*');
        isLowerPrecedence = arg2.isPlus();
        internalOperatorForm(arg2, isLowerPrecedence, symbolsAsFactoryMethod, depth, useOperators,
            usePrefix,
            noSymbolPrefix, text);
        return text.toString();
      } else if (isPlus()) {
  ...
  private void internalOperatorForm(IExpr arg1, boolean isLowerPrecedence,
      boolean symbolsAsFactoryMethod, int depth,
  ...
  /** {@inheritDoc} */
  @Override
  public boolean isNumericFunction(final boolean allowList) {
    if (head().isSymbol() && ((ISymbol) head()).isNumericFunctionAttribute()
      ...
      return forAll(new Predicate<IExpr>() {
        @Override
        public boolean test(IExpr x) {
          return x.isNumericFunction(allowList);
        }
      });
    }
    return false;
  ...
  /** {@inheritDoc} */
  @Override
  public boolean isNumericFunction(final IExpr expr) {
    if (head().isSymbol() && ((ISymbol) head()).isNumericFunctionAttribute() || isList()) {
      // check if all arguments are &quot;numeric&quot;
      return forAll(new Predicate<IExpr>() {
        @Override
        public boolean test(IExpr x) {
          return x.isNumericFunction(expr);
        }
      });
    }
    return false;
  ...
  /** {@inheritDoc} */
  @Override
  public boolean isNumericFunction(final Function<IExpr, String> list) {
    if (head().isSymbol() && ((ISymbol) head()).isNumericFunctionAttribute()
        || isList()
      ...
      return forAll(new Predicate<IExpr>() {
        @Override
        public boolean test(IExpr x) {
          return x.isNumericFunction(list);
        }
      });
    }
  ...
  public final boolean isPolynomialOfMaxDegree(ISymbol variable, long maxDegree) {
    return isPolynomialOfMaxDegree(S.List(variable), maxDegree);
  }
  ...
  /** {@inheritDoc} */
  @Override
  public final IASTMutable mapThreadEvaled(final EvalEngine engine, final IAST replacement,
      final int position) {
    final Function<IExpr, IExpr> function = new Function<IExpr, IExpr>() {
      @Override
      public IExpr apply(IExpr x) {
  ...
  /** {@inheritDoc} */
  @Override
  public IASTAppendable mapThreadEvaled(final EvalEngine engine, IASTAppendable appendAST,
      final IAST replacement,
      final int position) {
    final Function<IExpr, IExpr> function = new Function<IExpr, IExpr>() {
      @Override
      public IExpr apply(IExpr x) {
  ...
  /** {@inheritDoc} */
  @Override
  public IExpr normal(final boolean nilIfUnevaluated) {
    if (isConditionalExpression()) {
      return arg1();
    }
    IExpr temp = map(new Function<IExpr, IExpr>() {
      @Override
      public IExpr apply(IExpr x) {
        return x.normal(nilIfUnevaluated);
      }
    });
  ...
  public IExpr opposite() {
    ...
    return S.Times(F.CN1, this);
    // return F.eval(S.Times(F.CN1, this));
  }
  ...
  public final int patternHashCode() {
    if (size() > 1) {
      ...
      if (attr != ISymbol.NOATTRIBUTE) {
        if (ISymbolStatic.hasOrderlessFlatAttribute(attr)) {
          return 17 * head().hashCode();
        } else if (ISymbolStatic.hasFlatAttribute(attr)) {
...
//	}
  ...
  public final IExpr timesDistributed(final IExpr that) {
    ...
    return F.eval(S.Times(this, that));
  }
