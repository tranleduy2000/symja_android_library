...
public class WindowFunctions {
	...
	public static IExpr bartlettWindow(IExpr x) {
		return
		// [$ Piecewise({{1 - 2*x, 0 <= x <= 1/2}, {1 + 2*x, Inequality(-(1/2), LessEqual, x, Less, 0)}}, 0)
		// $]
		F.Piecewise(F.List(F.List(F.Plus(F.C1, F.Times(F.CN2, x)), F.LessEqual(F.C0, x, F.C1D2)),
				F.List(F.Plus(F.C1, F.Times(F.C2, x)), F.And(F.LessEqual(F.CN1D2, x), F.Less(x, F.C0)))), F.C0); // $$;
	}

	public static IExpr blackmanHarrisWindow(IExpr x) {
		return
		// [$ Piecewise({{(35875 + 48829*Cos(2*Pi*x) + 14128*Cos(4*Pi*x) + 1168*Cos(6*Pi*x))/100000, -(1/2) <= x <=
		// 1/2}}, 0)
		// $]
	...
	public static IExpr blackmanNuttallWindow(IExpr x) {
		return
		// [$ Piecewise({{(4891775*Cos(2*Pi*x) + 1365995*Cos(4*Pi*x) + 106411*Cos(6*Pi*x) + 3635819)/10000000, -(1/2) <=
		// x
		// <= 1/2}}, 0)
		// $]
		F.Piecewise(F.List(F.List(
	...
	public static IExpr flatTopWindow(IExpr x) {
		return
		// [$ Piecewise({{(215578947 + 416631580*Cos(2*Pi*x) + 277263158*Cos(4*Pi*x) + 83578947*Cos(6*Pi*x) +
	...
	public static IExpr gaussianWindow(IExpr x) {
		...
		F.Piecewise(F.List(
				F.List(F.Exp(F.Times(F.CN1, F.QQ(1L, 9L), F.ZZ(50L), F.Sqr(x))), F.LessEqual(F.CN1D2, x, F.C1D2))),
				F.C0); // $$;
	}
	...
	public static IExpr hammingWindow(IExpr x) {
		...
		F.Piecewise(F.List(F.List(F.Plus(F.QQ(25L, 46L), F.Times(F.QQ(21L, 46L), F.Cos(F.Times(F.C2, F.Pi, x)))),
				F.LessEqual(F.CN1D2, x, F.C1D2))), F.C0); // $$;
	}
	...
	public static IExpr nuttallWindow(IExpr x) {
		return
		// [$ Piecewise[{{(88942 + 121849*Cos(2*Pi*x) + 36058*Cos(4*Pi*x) + 3151*Cos(6*Pi*x))/250000, -(1/2) <= x <=
		// 1/2}}, 0] $]
		F.Piecewise(F.List(F.List(
	...
	public static IExpr parzenWindow(IExpr x) {
		return
		// [$ Piecewise({{-2*(-1 + 2*x)^3, Inequality(1/4, Less, x, LessEqual, 1/2)},{2*(1 + 2*x)^3, Inequality(-(1/2),
	...
	public static IExpr tukeyWindow(IExpr x) {
		return
		// [$ Piecewise({{1, -(1/3) - 2*x <= 0 && -(1/3) + 2*x <= 0}, {(1/2)*(1 + Cos(3*Pi*(1/6 + x))), x >= -(1/2) &&
		// -(1/3) - 2*x > 0}, {(1/2)*(1 + Cos(3*Pi*(-(1/6) + x))), -(1/3) + 2*x > 0 && x <= 1/2}}, 0) $]
		F.Piecewise(F.List(
				F.List(F.C1,
						F.And(F.LessEqual(F.Plus(F.CN1D3, F.Times(F.CN2, x)), F.C0),
								F.LessEqual(F.Plus(F.CN1D3, F.Times(F.C2, x)), F.C0))),
				F.List(F.Times(F.C1D2, F.Plus(F.C1, F.Cos(F.Times(F.C3, F.Pi, F.Plus(F.QQ(1L, 6L), x))))),
						F.And(F.GreaterEqual(x, F.CN1D2), F.Greater(F.Plus(F.CN1D3, F.Times(F.CN2, x)), F.C0))),
				F.List(F.Times(F.C1D2, F.Plus(F.C1, F.Cos(F.Times(F.C3, F.Pi, F.Plus(F.QQ(-1L, 6L), x))))),
