...
public class GraphFunctions {
      ...
      try {
        if (ast.isAST1()) {
          ...
          if (t != null) {
            ...
            if (edgeWeight.isList()) {
              GraphExpr<ExprWeightedEdge> gex = createWeightedGraph((IAST) ast.arg1(),
                  (IAST) ast.arg2(),
                  (IAST) edgeWeight);
              if (gex != null) {
                return gex;
    ...
    public IExpr evaluate(final IAST ast, EvalEngine engine) {
      try {
        if (ast.isAST1()) {
          ...
          Graph<IExpr, ExprEdge> gResult = new DefaultDirectedGraph<IExpr, ExprEdge>(null,
              new Supplier<ExprEdge>() {
                @Override
                public ExprEdge get() {
                  return new ExprEdge();
                }
  ...
  public static IAST graphToAdjacencyMatrix(final Graph<IExpr, ExprEdge> g) {
    Set<IExpr> vertexSet = g.vertexSet();
    int size = vertexSet.size();
  ...
  public static void graphToVisjs(Map<IExpr, Integer> map, StringBuilder buf,
      AbstractBaseGraph<IExpr, IntrusiveEdge> g) {
