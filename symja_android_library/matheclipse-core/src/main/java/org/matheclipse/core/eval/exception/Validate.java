...
public final class Validate {
    ...
    if (ast.get(pos) instanceof IntegerSym) {
      ...
      if (result == Integer.MIN_VALUE || 0 >= result) {
        ...
        String str = IOFunctions
            .getMessage("intpm", F.List(ast.topHead(), F.ZZ(pos)), EvalEngine.get());
        throw new ArgumentTypeException(str);
      }
      return result;
    }
  ...
  public static int checkIntLevelType(IExpr expr, int startValue) {
    ...
    if (expr.isReal()) {
      ...
      if (result == Integer.MIN_VALUE || startValue > result) {
        ...
        String str = IOFunctions
            .getMessage("intlevel", F.List(F.ZZ(startValue), expr), EvalEngine.get());
        throw new ArgumentTypeException(str);
      }
      return result;
    }
  ...
  public static IExpr checkSymbolType(IAST ast, int position, EvalEngine engine) {
    ...
    return IOFunctions
        .printMessage(ast.topHead(), "sym", F.List(ast.get(position), F.ZZ(position)), engine);
  }
