...
public class SeriesFunctions {
  ...
  private static final class Limit extends AbstractFunctionEvaluator implements LimitRules {
    ...
    private static IExpr lHospitalesRule(IExpr numerator, IExpr denominator, LimitData data) {
      ...
      try {
        ...
        if (data.limitValue.isInfinity() || data.limitValue.isNegativeInfinity()) {
          if (!numerator.isPower() && denominator.isPower() && denominator.exponent()
              .equals(F.C1D2)) {
    ...
    private static IExpr limitsInfinityOfRationalFunctions(ExprPolynomial numeratorPoly,
      ...
      return F
          .Divide(numeratorPoly.leadingBaseCoefficient(), denominatorPoly.leadingBaseCoefficient());
    }
        ...
        if (base.isFree(data.variable()) && !base.isZero()) {
          ...
          if (isInfinityLimit || data.limitValue().isNegativeInfinity()) {
            if (base.isNumericFunction(true)) {
              if (F.Greater.ofQ(F.Log(base), F.C0)) {
                return isInfinityLimit ? F.CInfinity : F.C0;
              }
            } else if (base.isNumericFunction(new Function<IExpr, String>() {
              @Override
              public String apply(IExpr s) {
                return s.isSymbol() ? "" : null;
              }
            })) {
        ...
        if (base.isTimes()) {
          IAST isFreeResult = ((IAST) base).partitionTimes(new Predicate<IExpr>() {
                                                             ...
                                                             public boolean test(IExpr x) {
                                                               return x.isFree(data.variable(), true);
                                                             }
                                                           }, F.C1, F.C1,
              F.List);
    ...
    private static IExpr substituteInfinity(final IAST arg1, LimitData data) {
      ...
      if (temp.isTimes()) {
        IExpr[] parts = Algebra
            .fractionalPartsTimesPower((IAST) temp, false, false, true, true, true, true);
        if (parts != null) {
    ...
    private static IExpr timesLimit(final IAST timesAST, final LimitData data) {
      ...
      IExpr[] parts = Algebra
          .fractionalPartsTimesPower(timesAST, false, false, true, true, true, true);
      if (parts == null) {
        IAST[] timesPolyFiltered = timesAST.filter(new Predicate<IExpr>() {
          @Override
          public boolean test(IExpr x) {
            return x.isPolynomial(data.variable);
          }
        });
        if (timesPolyFiltered[0].size() > 1 && timesPolyFiltered[1].size() > 1) {
          ...
          if (first.isReal() || second.isReal()) {
            ...
            if (data.limitValue().isZero()) {
              ...
              IExpr newTimes = timesAST
                  .replaceAll(F.Rule(data.variable, F.Power(data.variable, F.CN1)));
    ...
    private Function<IExpr, IExpr> normal(final IAST heads) {
      return new Function<IExpr, IExpr>() {
        ...
        public IExpr apply(IExpr x) {
          ...
          if (heads.exists(new Predicate<IExpr>() {
            @Override
            public boolean test(IExpr y) {
              return y.equals(head);
            }
          })) {
            return x.normal(true);
          }
