...
ISetDelayed(688,FixIntRules($p("rulelist")),
    Block(List(Integrate,$rubi("Integrate::Subst"),$rubi("Integrate::Simp"),$rubi("Integrate::Dist")),CompoundExpression(SetAttributes(List($rubi("Integrate::Simp"),$rubi("Integrate::Dist"),Integrate,$rubi("Integrate::Subst")),$s("HoldAll")),Map(Function(FixIntRule(Slot1,Part(Slot1,C1,C1,C2,C1))),$s("rulelist"))))),
...
ISetDelayed(708,FixRhsIntRule(Times(a_,u_),x_),
    Condition(Dist(a,u,x),MemberQ(List(Integrate,$rubi("Integrate::Subst")),Head(Unevaluated(u))))),
ISetDelayed(709,FixRhsIntRule(u_,x_),
    If(And(SameQ(Head(Unevaluated(u)),$rubi("Integrate::Dist")),Equal(Length(Unevaluated(u)),C2)),Insert(Unevaluated(u),x,C3),If(MemberQ(List(Integrate,$rubi("Integrate::Unintegrable"),$rubi("Integrate::CannotIntegrate"),$rubi("Integrate::Subst"),$rubi("Integrate::Simp"),$rubi("Integrate::Dist")),Head(Unevaluated(u))),u,Simp(u,x))))
  );
}
