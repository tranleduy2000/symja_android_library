...
public class TensorFunctions {
  ...
  private static class ListCorrelate extends AbstractEvaluator {
    ...
    public static IExpr listCorrelate(final IAST kernel, int kernelSize, final IAST tensor,
        int tensorSize) {
  ...
  private static class TensorRank extends AbstractEvaluator {
    ...
    public IExpr evaluate(final IAST ast, EvalEngine engine) {
      if (ast.arg1().isList()) {
        IAST list = (IAST) ast.arg1();
        List<Integer> intList = LinearAlgebra
            .dimensions((IAST) ast.arg1(), list.head(), Integer.MAX_VALUE);
