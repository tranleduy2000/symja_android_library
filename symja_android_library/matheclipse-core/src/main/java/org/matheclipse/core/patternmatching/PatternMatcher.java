...
public class PatternMatcher extends IPatternMatcher implements Externalizable {
  ...
  private final class OrderlessMatcher {
     ...
     * @return
     */
    public boolean matchOrderlessAST(final int lhsPosition, final StackMatcher stackMatcher,
          ...
          if (fLHSPatternAST.forAll(new ObjIntPredicate<IExpr>() {
            ...
            try {
              if (localStackMatcher.push(subPattern, temp)) {
                ...
                if (OrderlessMatcher.this
                    .matchOrderlessAST(lhsPosition + 1, localStackMatcher, engine)) {
                  matched = true;
                  return true;
                }
              }
  ...
  public IPatternMap createPatternMap() {
    if (fPatternMap == null) {
      ...
      fPatternMap = IPatternMapStatic.determinePatterns(fLhsPatternExpr, priority, null);
    }
    return fPatternMap;
  }
  ...
  public IPatternMap determinePatterns(int[] priority) {
    return IPatternMapStatic.determinePatterns(fLhsPatternExpr, priority, null);
  }
  ...
  // objc-changed
  @AutoreleasePool
  private void invokeMultisetPartitionsIteratorExecute(FlatOrderlessStepVisitor visitor,
      IAST lhsPatternAST, ObjectRef<Boolean> resultRef) {
    MultisetPartitionsIterator iter = new MultisetPartitionsIterator(visitor,
...
//         }
    // objc-changed: memory issue
    if (existImpl(symbolWithDefaultValue, lhsPatternAST,
        engine, defaultValueMatched, cloned, fPatternMap)) {
      return F.NIL;
    }
  ...
  private boolean existImpl(final ISymbol symbolWithDefaultValue, IAST lhsPatternAST,
      final EvalEngine engine,
      boolean[] defaultValueMatched, IASTAppendable cloned, IPatternMap fPatternMap) {
    final int size = lhsPatternAST.size();
    int startOffset = 1;
    for (int i = startOffset; i < size; i++) {
      IExpr temp = lhsPatternAST.get(i);
      if (existImpl1(symbolWithDefaultValue,
          engine, defaultValueMatched, cloned, fPatternMap,
          temp, i)) {
        return true;
      }
  ...
  private boolean existImpl1(final ISymbol symbolWithDefaultValue, final EvalEngine engine,
      boolean[] defaultValueMatched, IASTAppendable cloned, IPatternMap fPatternMap, IExpr temp,
      int i) {
    if (temp.isPatternDefault()) {
      if (temp.isOptional()) {
        IAST optional = (IAST) temp;
  ...
  private boolean matchASTSpecialBuiltIn(
      IAST lhsPatternAST, final IExpr lhsEvalExpr, final EvalEngine engine, StackMatcher stackMatcher) {
    int functionID = lhsPatternAST.headID();
    if (functionID >= ID.Alternatives && functionID <= ID.Verbatim) {
      ...
      switch (functionID) {
        ...
        case ID.Condition:
          if (lhsPatternAST.isCondition()) {
            ...
            try {
              ...
              final PatternMatcher matcher = new PatternMatcherEvalEngine(lhsTempPatternExpr,
                  engine);
              ...
              if (lhsEvalExpr.isRational()) {
                ...
                matched =
                    matchExpr(lhsPatternAST.arg1(), rational.numerator(), engine, stackMatcher)
                        && matchExpr(lhsPatternAST.arg2(), rational.denominator(), engine,
                        stackMatcher);
                return matched;
              }
              matched = matchASTExpr(lhsPatternAST, lhsEvalExpr, engine, stackMatcher);
        ...
        case ID.PatternTest:
          ...
          break;
        case ID.Verbatim:
          if (lhsPatternAST.isAST(F.Verbatim, 2)) {
  ...
  /**
   * Match Flat or Orderless LHS pattern expressions. It's assumed that the headers of the
   * expreesion already matched.
   *
   * @param sym
   * @param lhsPattern
   * @param lhsEval
   ...
   * @return
   */
  private boolean matchFlatAndFlatOrderlessAST(final ISymbol sym, IAST lhsPattern, IAST lhsEval,
      EvalEngine engine,
      StackMatcher stackMatcher) {
...
//
      ...
      FlatStepVisitor visitor = new FlatStepVisitor(sym, lhsPatternAST, lhsEvalAST, stackMatcher,
          fPatternMap);
  ...
  // j2objc changed: auto release pool
  @AutoreleasePool
  private void numberPartitionsIteratorExecuteImpl(FlatStepVisitor visitor, IAST lhsEvalAST,
      IAST lhsPatternAST, ObjectRef<Boolean> resultRef) {
