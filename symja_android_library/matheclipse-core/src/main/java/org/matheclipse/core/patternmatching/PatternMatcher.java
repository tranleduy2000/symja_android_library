...
public class PatternMatcher extends IPatternMatcher implements Externalizable {
  ...
  private boolean matchAlternatives(
      IAST lhsPatternAlternatives, final IExpr lhsEvalExpr, final EvalEngine engine) {
    boolean matched = false;
    final IExpr[] patternValues = fPatternMap.copyPattern();
    try {
      matched = lhsPatternAlternatives.exists(new Predicate<IExpr>() {
        @Override
...
//         }
  ...
  private boolean matchFlat(
    ...
    ObjectRef<Boolean> resultRef = new ObjectRef<>();
    numberPartitionsIteratorExecuteImpl(visitor, lhsEvalAST, lhsPatternAST, resultRef);
    return !resultRef.get();
  }
