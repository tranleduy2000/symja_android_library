...
public class Integrate extends AbstractFunctionEvaluator {
  ...
  public IExpr evaluate(final IAST holdallAST, final EvalEngine engine) {
    ...
    try {
      ...
      if (arg1.isAST()) {
        ...
        if (fx.isTimes()) {
          IAST[] temp = ((IAST) fx).filter(new Predicate<IExpr>() {
            @Override
  ...
  private static IExpr definiteIntegral(IExpr function, IAST xValueList, EvalEngine engine) {
    ...
    if (a.isNegativeResult() && b.isPositiveResult()) {
      ...
      IExpr FZeroAbove = engine
          .evaluate(F.Limit(function, F.Rule(x, F.C0), F.Rule(F.Direction, F.CN1)));
      if (!FZeroAbove.isFree(F.DirectedInfinity, true) || !FZeroAbove
          .isFree(F.Indeterminate, true)) {
        return engine.printMessage("Not integrable: " + function + " for limit " + x + " -> 0");
      }
      IExpr FZeroBelow = engine
          .evaluate(F.Limit(function, F.Rule(x, F.C0), F.Rule(F.Direction, F.C1)));
      if (!FZeroBelow.isFree(F.DirectedInfinity, true) || !FZeroBelow
          .isFree(F.Indeterminate, true)) {
        return engine.printMessage("Not integrable: " + function + " for limit " + x + " -> 0");
      }
  ...
  private static IExpr integrateByRubiRules(IAST arg1, IExpr x, IAST ast, EvalEngine engine) {
    ...
    if (arg1.isFreeAST(new Predicate<IExpr>() {
      @Override
      public boolean test(IExpr s) {
        return s.isSymbol() && ((ISymbol) s).isContext(Context.RUBI);
      }
    })) {
        ...
        try {
          ...
          try {
            ...
            return engine.printMessage("Integrate Rubi recursion limit "
                + Config.INTEGRATE_RUBI_RULES_RECURSION_LIMIT + " RuntimeException: " + ast
                .toString());
...
//		}
    ...
    F.ISet(F.$s("§$calculusfunctions"),
        F.List(F.D, F.Sum, F.Product, F.Integrate, F.$rubi("Unintegrable"),
    ...
    F.ISet(UtilityFunctionCtors.FractionalPowerQ, //
        F.Function(
            F.And(F.SameQ(F.Head(F.Slot1), F.Power),
