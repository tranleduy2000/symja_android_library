...
public class Integrate extends AbstractFunctionEvaluator {
  ...
  @ObjcMemoryIssue
  @Override
  public IExpr evaluate(IAST holdallAST, final EvalEngine engine) {
    ...
    try {
      ...
      if (holdallAST.size() > 3) {
        ...
        return holdallAST.foldRight(new BiFunction<IExpr, IExpr, IExpr>() {
          ...
          public IExpr apply(IExpr x, IExpr y) {
            return engine.evaluateNIL(F.Integrate(x, y));
          }
        }, arg1, 2);
      }
      ...
      if (arg1.isAST()) {
        ...
        if (fx.isTimes()) {
          IAST[] temp = fx.filter(new Predicate<IExpr>() {
            @Override
...
//    }
    ...
    setOptions(newSymbol, F.List(F.Rule(F.Assumptions, F.$Assumptions)));
  }
