...
public class Solve extends AbstractFunctionEvaluator {
  ...
  protected static class ExprAnalyzer implements Comparable<ExprAnalyzer> {
    ...
    private void getPlusArgumentEquationType(IExpr eqExpr) {
      if (eqExpr.isTimes()) {
        ...
        for (int i = 1; i < arg.size(); i++) {
          ...
          } else if (expr.isPower() && (expr.base().isInteger() || expr.exponent()
              .isNumIntValue())) {
            if (fEquationType == LINEAR) {
              fEquationType = POLYNOMIAL;
            }
    ...
    private IExpr rewriteInverseFunction(IAST ast, IExpr arg1) {
      if (ast.isAbs()) {
        return fEngine.evaluate(
            F.Expand(F.Times(F.Subtract(ast.arg1(), F.Times(F.CN1, arg1)),
                F.Subtract(ast.arg1(), arg1))));
      } else if (ast.isAST1()) {
      ...
      } else if (ast.isTimes() && ast.size() == 3 && ast.first().isNumericFunction() && ast.second()
          .isAST1()) {
        ...
        if (expr.isAST()) {
          ...
          if (inverseFunction.isPresent()) {
            ...
            if (function.arg2().isPower()) {
              ...
              IExpr temp = rewritePowerFractions(plusAST, i, (INumber) function.arg1(),
                  power.base(),
                  power.exponent());
              if (temp.isPresent()) {
              ...
              IAST inverseTimesFunction = InverseFunction
                  .getUnaryInverseFunction((IAST) function.arg2());
     ...
     * @return
     */
    private IExpr rewritePowerFractions(IAST plusAST, int i, INumber num, IExpr base,
        IExpr exponent) {
    ...
    protected void simplifyAndAnalyze() {
      ...
      } else if (fNumerator.isTimes() && !fNumerator
          .isFree(Predicates.in(fListOfVariables), true)) {
  ...
  protected static IASTAppendable analyzeSublist(ArrayList<ExprAnalyzer> analyzerList,
      IAST variables,
      IASTAppendable resultList, int maximumNumberOfResults, IASTAppendable matrix,
   ...
   * @param expr
   * @param engine
   * @param evalTogether evaluate <code>Together[expr]</code> before determining numerator and denominator of the expression.
        ...
        if (ast.isAST3()) {
          ...
          if (domain.equals(F.Booleans)) {
            return BooleanFunctions
                .solveInstances(ast.arg1(), userDefinedVariables, Integer.MAX_VALUE);
          }
  ...
  private static IExpr solveRecursive(IASTMutable termsEqualZeroList, IASTMutable inequationsList,
    ...
    if (inequationsList.isEmpty() && termsEqualZeroList.size() == 2 && variables.size() == 2) {
      ...
      if (!res.isList() || !res.isFree(new Predicate<IExpr>() {
        @Override
    ...
    if (termsEqualZeroList.size() > 2 && variables.size() >= 3) {
      ...
      IExpr firstVariable = variables.arg1();
      IAST[] reduced = Eliminate
          .eliminateOneVariable(F.List(F.Equal(firstEquation, F.C0)), firstVariable,
              engine);
      if (reduced != null) {
  ...
  private static IAST eliminateOneVariable(IAST termsEqualZeroList, IExpr variable,
    ...
    if (tempAST != null && tempAST[1] != null && tempAST[1].isRule() && tempAST[1].second()
        .isTrue()) {
      return F.CEmptyList;
    }
    ...
    try {
      ...
      if (vector.size() > 1) {
        ...
        if (augmentedMatrix != null) {
          IAST subSolutionList = LinearAlgebra
              .rowReduced2RulesList(augmentedMatrix, variables, resultList,
                  engine);
  ...
  private static IASTMutable solveTimesEquationsRecursively(IASTMutable termsEqualZeroList,
      ...
      boolean numericFlag, IAST variables, EvalEngine engine) {
    try {
      IASTMutable resultList = solveEquations(termsEqualZeroList, inequationsList, variables, 0,
          engine);
      ...
      for (int i = 1; i < termsEqualZeroList.size(); i++) {
        ...
        if (termEQZero.isTimes()) {
          ...
          if (termEQZero.isAST()) {
            ...
            if (termEQZero.isTimes()) {
              solveTimesAST((IAST) termEQZero, termsEqualZeroList, inequationsList, numericFlag,
                  variables, engine, subSolutionSet, i);
            }
          }
        }
      }
  ...
  private static void solveTimesAST(IAST times, IAST termsEqualZeroList, IAST inequationsList,
      boolean numericFlag,
      IAST variables, EvalEngine engine, Set<IExpr> subSolutionSet, int i) {
    IAST temp;
          ...
          if (clonedEqualZeroList.size() == 2 && variables.size() == 2) {
            ...
            if (!res.isPresent()) {
              if (numericFlag) {
                ...
                res = F.FindRoot
                    .ofNIL(engine, clonedEqualZeroList.arg1(), F.List(firstVariable, F.C0));
              }
            }
            if (!res.isList() || !res.isFree(new Predicate<IExpr>() {
              @Override
