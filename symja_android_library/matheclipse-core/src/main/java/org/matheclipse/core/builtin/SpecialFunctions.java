...
public class SpecialFunctions {
        ...
        try {
          ...
          if (bInt > 0) {
            ...
            if (a.isOne()) {
              return
                  // [$ ( (1/n)*(1 - (1 - z)^n) ) $]
                  F.Times(F.Power(n, F.CN1),
                      F.Subtract(F.C1, F.Power(F.Subtract(F.C1, z), n))); // $$;
            }
            // if (bInt <= 1) {
            // ISymbol k = F.Dummy("k");
            // return
    ...
    public IExpr evaluate(final IAST ast, EvalEngine engine) {
      ...
      if (a.isNumber()) {
        if (a.isZero() && s.isInteger() && s.isNegative()) {
          ...
          return F
              .Times(F.CN1, F.Divide(F.BernoulliB(F.Plus(1, s.negate())), F.Plus(1, s.negate())));
        }
        if (a.isOne()) {
  ...
  private static class LogGamma extends AbstractTrigArg1 implements INumeric {
    ...
    public IExpr evaluateArg1(final IExpr arg1, EvalEngine engine) {
      ...
      if (arg1.isNegativeInfinity() || arg1.isDirectedInfinity(F.CI) || arg1
                        ...
                        switch (q) {
                          case 1:
                            ...
                            return
                                ...
                                F.Times(F.Power(z,
                                    F.Plus(b1, F.Times(F.C1D2, F.Plus(F.Negate(b1), b2)))),
  ...
  private static final class ProductLog extends AbstractArg12 implements ProductLogRules {
    ...
    public IExpr e1DblArg(final INum d) {
      ...
      try {
        return F.num(ApfloatMath
            .w(new Apfloat(new BigDecimal(d.doubleValue()), FEConfig.MACHINE_PRECISION)));
      } catch (Exception ce) {
  ...
  private static class StieltjesGamma extends AbstractFunctionEvaluator implements
      StieltjesGammaRules {
