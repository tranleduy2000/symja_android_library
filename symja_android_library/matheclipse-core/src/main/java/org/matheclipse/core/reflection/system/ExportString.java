...
public class ExportString extends AbstractEvaluator {
  ...
  public IExpr evaluate(final IAST ast, EvalEngine engine) {
    if (Config.isFileSystemEnabled(engine)) {
      ...
      try {
        ...
        return engine
            .printMessage("ExportString: format: " + arg1.toString() + " - " + ex.getMessage());
      } finally {
        if (writer != null) {
          try {
            writer.close();
  ...
  private static final ComponentNameProvider<IExpr> nameProvider = new ComponentNameProvider<IExpr>() {
    @Override
    public String getName(IExpr v) {
  ...
  void graphExport(Graph<IExpr, DefaultEdge> g, Writer writer, Extension format)
    ...
    switch (format) {
      case DOT:
        DOTExporter<IExpr, DefaultEdge> dotExporter = new DOTExporter<>(
            new IntegerComponentNameProvider<IExpr>(), null,
            null, null, null);
