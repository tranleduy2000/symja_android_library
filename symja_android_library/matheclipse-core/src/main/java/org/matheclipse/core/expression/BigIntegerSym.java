...
public class BigIntegerSym extends AbstractIntegerSym {
  ...
  public IInteger multiply(final IInteger that) {
    ...
    OperationSystem.checkMemory(
        calculateApproximatelySizeOf(fBigIntValue) + calculateApproximatelySizeOf(that));
  ...
  public IRational multiply(IRational parm1) {
    ...
    OperationSystem.checkMemory(
        calculateApproximatelySizeOf(fBigIntValue) + calculateApproximatelySizeOf(
            parm1.toBigNumerator()));
    OperationSystem
        .checkMultiplicationOperation(fBigIntValue.bitLength(), parm1.toBigNumerator().bitLength());
    if (parm1.isZero()) {
      return F.C0;
    }
    if (parm1.isOne()) {
