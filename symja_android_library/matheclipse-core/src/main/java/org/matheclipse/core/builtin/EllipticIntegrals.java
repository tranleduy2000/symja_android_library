...
public class EllipticIntegrals {
      ...
      if (x.equals(y)) {
        ...
        return F.Piecewise(
            F.List(
                F.List(
                    F.CComplexInfinity,
                    F.And(F.LessEqual(F.Re(x), F.C0), F.Equal(F.Im(x), F.C0)))),
            F.Power(x, F.CN1D2));
      }
      if (y.isZero()) {
        return F.CComplexInfinity;
  ...
  private static class CarlsonRG extends AbstractFunctionEvaluator {
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      ...
      if (x.equals(y)) {
        ...
        return S.Times
            .of(engine, F.C1D2, F.Plus(F.Sqrt(z), F.Times(x, F.CarlsonRF(x, x, z))));
      }
      if (engine.isDoubleMode()) {
        try {
          double xd = Double.NaN;
      ...
      if (engine.isDoubleMode()) {
        try {
          ...
          if (Double.isNaN(zDouble)) {
            Complex zc = z.evalComplex();
            return F.complexNum(
                EllipticIntegralsJS.ellipticE(new Complex(Math.PI / 2.0), zc));
          } else {
  ...
  private static class EllipticK extends AbstractFunctionEvaluator {
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      ...
      IExpr m = ast.arg1();
      if (m.isInfinity() || m.isNegativeInfinity() || m.isDirectedInfinity(F.CI) || m
          .isDirectedInfinity(F.CNI)) {
        return F.C0;
      }
      if (m.isZero()) {
        return F.CPiHalf;
      ...
      if (ast.isAST3()) {
        ...
        if (engine.isDoubleMode()) {
          try {
            ...
            if (Double.isNaN(nDouble) || Double.isNaN(zDouble) || Double
                .isNaN(mDouble)) {
              ...
              return F.complexNum(
                  EllipticIntegralsJS.ellipticPi(nDouble, zDouble, mDouble));
            }
          } catch (ValidateException ve) {
            if (FEConfig.SHOW_STACKTRACE) {
              ve.printStackTrace();
      ...
      if (engine.isDoubleMode()) {
        try {
          ...
          if (Double.isNaN(nDouble) || Double.isNaN(mDouble)) {
            ...
            Complex mc = m.evalComplex();
            return F.complexNum(
                EllipticIntegralsJS.ellipticPi(nc, new Complex(Math.PI / 2.0), mc));
          } else {
            return F.complexNum(
                EllipticIntegralsJS.ellipticPi(nDouble, Math.PI / 2.0, mDouble));
          }
        } catch (ValidateException ve) {
          if (FEConfig.SHOW_STACKTRACE) {
            ve.printStackTrace();
  ...
  private static class EllipticTheta extends AbstractFunctionEvaluator {
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      ...
      if (ast.isAST3()) {
        ...
        if (a >= 1 && a <= 4) {
          ...
          if (x.isReal() && m.isReal()) {
            try {
              return F
                  .complexNum(EllipticFunctionsJS
                      .jacobiTheta(a, x.evalDouble(), m.evalDouble()));
            } catch (RuntimeException rex) {
              return engine.printMessage(ast.topHead(), rex);
            }
            ...
            try {
              return F
                  .complexNum(EllipticFunctionsJS
                      .jacobiTheta(a, x.evalComplex(), m.evalComplex()));
            } catch (RuntimeException rex) {
              return engine.printMessage(ast.topHead(), rex);
            }
          }
      ...
      if (a >= 1 && a <= 4) {
        ...
        if (m.isReal()) {
          try {
            return F
                .complexNum(EllipticFunctionsJS.jacobiTheta(a, 0.0, m.evalDouble()));
          } catch (RuntimeException rex) {
            return engine.printMessage(ast.topHead(), rex);
          }
          ...
          try {
            return F
                .complexNum(EllipticFunctionsJS
                    .jacobiTheta(a, org.hipparchus.complex.Complex.ZERO,
                        m.evalComplex()));
          } catch (RuntimeException rex) {
            return engine.printMessage(ast.topHead(), rex);
          }
    ...
    public void setUp(final ISymbol newSymbol) {
      newSymbol
          .setAttributes(ISymbol.LISTABLE | ISymbol.NHOLDFIRST | ISymbol.NUMERICFUNCTION);
      super.setUp(newSymbol);
    }
  }
...
//
      ...
      if (z.isInexactNumber() && m.isInexactNumber()) {
        try {
          if (z.isReal() && m.isReal()) {
            return F
                .complexNum(EllipticFunctionsJS
                    .jacobiAmplitude(z.evalDouble(), m.evalDouble()));
          }
          return F
              .complexNum(
  ...
  private static class JacobiCD extends AbstractFunctionEvaluator {
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      ...
      try {
        ...
        if (Double.isNaN(zDouble) || Double.isNaN(mDouble)) {
          return F
              .complexNum(EllipticFunctionsJS.jacobiCD(z.evalComplex(), m.evalComplex()));
        } else {
  ...
  private static class JacobiCN extends AbstractFunctionEvaluator {
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      ...
      try {
        ...
        if (Double.isNaN(zDouble) || Double.isNaN(mDouble)) {
          return F
              .complexNum(EllipticFunctionsJS.jacobiCN(z.evalComplex(), m.evalComplex()));
        } else {
  ...
  private static class JacobiDN extends AbstractFunctionEvaluator {
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      ...
      try {
        ...
        if (Double.isNaN(zDouble) || Double.isNaN(mDouble)) {
          return F
              .complexNum(EllipticFunctionsJS.jacobiDN(z.evalComplex(), m.evalComplex()));
        } else {
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      ...
      if (engine.isDoubleMode()) {
        try {
          ...
          if (Double.isNaN(zDouble) || Double.isNaN(mDouble)) {
            return F.complexNum(
                EllipticFunctionsJS.jacobiSC(z.evalComplex(), m.evalComplex()));
          } else {
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      ...
      if (engine.isDoubleMode()) {
        try {
          ...
          if (Double.isNaN(zDouble) || Double.isNaN(mDouble)) {
            return F.complexNum(
                EllipticFunctionsJS.jacobiSD(z.evalComplex(), m.evalComplex()));
          } else {
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      ...
      if (engine.isDoubleMode()) {
        try {
          ...
          if (Double.isNaN(zDouble) || Double.isNaN(mDouble)) {
            return F.complexNum(
                EllipticFunctionsJS.jacobiSN(z.evalComplex(), m.evalComplex()));
          } else {
  ...
  private static class KleinInvariantJ extends AbstractFunctionEvaluator implements
      ...
      if (ast.arg2().isList2()) {
        ...
        if (u.isInexactNumber() && g2.isInexactNumber() && g3.isInexactNumber()) {
          try {
            return F.complexNum(
                EllipticFunctionsJS
                    .weierstrassP(u.evalComplex(), g2.evalComplex(), g3.evalComplex()));
          } catch (RuntimeException rex) {
            return engine.printMessage(ast.topHead(), rex);
          }
        }
      ...
      if (ast.arg2().isList2()) {
        ...
        if (u.isInexactNumber() && g2.isInexactNumber() && g3.isInexactNumber()) {
          try {
            return F
                .complexNum(EllipticFunctionsJS
                    .weierstrassPPrime(u.evalComplex(), g2.evalComplex(),
                        g3.evalComplex()));
          } catch (RuntimeException rex) {
            return engine.printMessage(ast.topHead(), rex);
          }
