  ...
  private static class EllipticE extends AbstractFunctionEvaluator {
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      ...
      if (z.isMinusOne()) {
        ...
        return F
            .Times(F.C1D2, F.C1DSqrt2, F.Power(F.Pi, F.CN1D2), F.Power(F.Gamma(F.QQ(3L, 4L)), -2),
                F.Plus(F.Sqr(F.Pi), F.Times(F.C2, F.Power(F.Gamma(F.QQ(3L, 4L)), 4))));
      }
      if (engine.isDoubleMode()) {
        try {
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      ...
      if (a >= 1 && a <= 4) {
        ...
        if (m.isReal()) {
          ...
          try {
            return F
                .complexNum(EllipticFunctionsJS.jacobiTheta(a, org.hipparchus.complex.Complex.ZERO,
                    m.evalComplex()));
          } catch (RuntimeException rex) {
            return engine.printMessage(ast.topHead(), rex);
          }
    ...
    public IExpr evaluate(IAST ast, EvalEngine engine) {
      ...
      if (ast.arg2().isList2()) {
        ...
        if (u.isInexactNumber() && g2.isInexactNumber() && g3.isInexactNumber()) {
          try {
            return F
                .complexNum(EllipticFunctionsJS.weierstrassPPrime(u.evalComplex(), g2.evalComplex(),
                    g3.evalComplex()));
          } catch (RuntimeException rex) {
            return engine.printMessage(ast.topHead(), rex);
          }
