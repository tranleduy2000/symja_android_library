...
public class ClusteringFunctions {
	...
	private final static class ChessboardDistance extends AbstractDistance {
		...
		@Override
		public IExpr distance(IExpr a, IExpr b) {
			final IAST vect1 = (IAST) a.normal(false);
			final IAST vect2 = (IAST) b.normal(false);
			IASTAppendable maxAST = F.Max();
			return maxAST.appendArgs(a.size(), new IntFunction<IExpr>() {
				@Override
				public IExpr apply(int i) {
					return F.Abs(F.Subtract(vect1.get(i), vect2.get(i)));
				}
			});
		}
	...
	private final static class EuclideanDistance extends AbstractDistance {
		...
		public IExpr distance(IExpr a, IExpr b) {
			...
			plusAST.appendArgs(size, new IntFunction<IExpr>() {
				@Override
				public IExpr apply(int i) {
					return F.Sqr(F.Abs(F.Subtract(vect1.get(i), vect2.get(i))));
				}
			});
