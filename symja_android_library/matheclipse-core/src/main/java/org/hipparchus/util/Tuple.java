 ...
 * @since 1.2
 */
public class Tuple extends CalculusFieldElementImpl<Tuple> implements CalculusFieldElement<Tuple> {
   ...
   */
  private Tuple(final TupleField field,
      final double[] x) {// NOPMD - storing user-supplied array is intentional and documented here
    this.values = x;
    this.field = field;
  }
  ...
  public boolean equals(final Object obj) {
    if (obj instanceof Tuple) {
      ...
      if (getDimension() == that.getDimension()) {
        ...
        for (int i = 0; i < values.length; ++i) {
          equals &=
              Double.doubleToRawLongBits(values[i]) == Double.doubleToRawLongBits(that.values[i]);
        }
        return equals;
      }
    }
