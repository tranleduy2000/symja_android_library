 ...
 * @since 1.2
 */
public class FieldTuple<T extends CalculusFieldElement<T>> extends
    CalculusFieldElementImpl<FieldTuple<T>>
   ...
   */
  private FieldTuple(final FieldTupleField<T> field,
      final T[] x) { // NOPMD - storing user-supplied array is intentional and documented here
    this.values = x;
    this.field = field;
  }
  ...
  public FieldTuple<T> newInstance(final double value) {
    @SuppressWarnings("unchecked") final FieldTuple<T> t = new FieldTuple<>(field,
        (T[]) MathArrays.buildArray(field, values.length));
  ...
  public FieldTuple<T> add(final FieldTuple<T> a) {
    final FieldTuple<T> result = new FieldTuple<>(field,
        MathArrays.buildArray(values[0].getField(), values.length));
  ...
  public boolean equals(final Object obj) {
    if (obj instanceof FieldTuple<?>) {
      @SuppressWarnings("unchecked") final FieldTuple<T> that = (FieldTuple<T>) obj;
  ...
  public FieldSinCos<FieldTuple<T>> sinCos() {
    final FieldTuple<T> sin = new FieldTuple<>(field,
        MathArrays.buildArray(values[0].getField(), values.length));
    final FieldTuple<T> cos = new FieldTuple<>(field,
        MathArrays.buildArray(values[0].getField(), values.length));
    for (int i = 0; i < values.length; ++i) {
  ...
  public FieldSinhCosh<FieldTuple<T>> sinhCosh() {
    final FieldTuple<T> sinh = new FieldTuple<>(field,
        MathArrays.buildArray(values[0].getField(), values.length));
    final FieldTuple<T> cosh = new FieldTuple<>(field,
        MathArrays.buildArray(values[0].getField(), values.length));
    for (int i = 0; i < values.length; ++i) {
   ...
   * @param <T> the type of the field elements
   */
  private static class FieldTupleField<T extends CalculusFieldElement<T>> implements
      Field<FieldTuple<T>> {
    ...
    public boolean equals(final Object other) {
      if (other instanceof FieldTupleField) {
        @SuppressWarnings("unchecked") final FieldTupleField<T> that = (FieldTupleField<T>) other;
