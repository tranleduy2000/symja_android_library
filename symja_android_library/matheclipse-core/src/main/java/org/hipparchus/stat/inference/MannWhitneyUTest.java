...
public class MannWhitneyUTest {
    ...
    private double varU(final int n, final int m,
        ...
        final long tSum = new StreamWrapper<>(tiesMap.entrySet()).stream()
            .mapToLong(new ToLongFunction<Map.Entry<Double, Integer>>() {
                @Override
                public long applyAsLong(Map.Entry<Double, Integer> e) {
                    return e.getValue() * e.getValue() * e.getValue() -
                            e.getValue();
                }
            })
            .sum();
    ...
    private Map<Double, Integer> tiesMap(final double[] x, final double[] y) {
        ...
        for (int i = 0; i < x.length; i++) {
            MapWrapper.merge(tiesMap, x[i], 1, new BiFunction<Integer, Integer, Integer>() {
                @Override
        ...
        for (int i = 0; i < y.length; i++) {
            MapWrapper.merge(tiesMap, y[i], 1, new BiFunction<Integer, Integer, Integer>() {
                @Override
                public Integer apply(Integer a, Integer b) {
                    return DInteger.sum(a, b);
                }
        ...
        new SetWrapper<>(tiesMap.entrySet()).removeIf(new Predicate<Map.Entry<Double, Integer>>() {
            @Override
            public boolean test(Map.Entry<Double, Integer> e) {
                return e.getValue() == 1;
            }
        });
        return tiesMap;
