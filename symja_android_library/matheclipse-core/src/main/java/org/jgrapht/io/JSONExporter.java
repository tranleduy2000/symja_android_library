    ...
    GraphExporter<V, E>
{
    ...
    public JSONExporter()
    {
        this(new IntegerComponentNameProvider<V>());
    }
    ...
    {
        this(
            vertexIDProvider, new EmptyComponentAttributeProvider<V>(),
            new IntegerComponentNameProvider<E>(), new EmptyComponentAttributeProvider<E>());
    }
        ...
        ComponentAttributeProvider<E> edgeAttributeProvider)
    {
        ...
        this.vertexAttributeProvider = DObjects.requireNonNull(vertexAttributeProvider);
        this.edgeAttributeProvider = DObjects.requireNonNull(edgeAttributeProvider);
    }

    @Override
    public void exportGraph(Graph<V, E> g, OutputStream out) throws ExportException {
        exportGraph(g, new OutputStreamWriter(out, Charset.forName("UTF-8")));
    }

    @Override
    public void exportGraph(Graph<V, E> g, File file) throws ExportException {
        try {
    ...
    {
        for (Map.Entry<String, Attribute> e : vertexAttributeProvider
                .getComponentAttributes(v).entrySet()) {
            if (!e.getKey().equals("id")) {
                out.print(",");
                out.print(quoted(e.getKey()));
                out.print(":");
                outputValue(out, e.getValue());
            }
        }
    }
    ...
    private void exportEdgeAttributes(PrintWriter out, Graph<V, E> g, E e)
    {
        ...
        for (Map.Entry<String, Attribute> entry : edgeAttributeProvider
                .getComponentAttributes(e).entrySet()) {
    ...
    private void outputValue(PrintWriter out, Attribute value)
    {
        ...
        if (type.equals(AttributeType.BOOLEAN)) {
            ...
            if (!DDouble.isFinite(doubleValue)) {
                throw new IllegalArgumentException("Infinity and NaN not allowed in JSON");
            }
