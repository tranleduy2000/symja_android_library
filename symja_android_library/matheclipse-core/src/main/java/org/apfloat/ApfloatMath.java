...
public class ApfloatMath {
   ...
   * @throws java.lang.ArithmeticException If both <code>x</code> and <code>n</code> are zero.
   */
   ...
   * @throws java.lang.ArithmeticException If <code>x</code> is negative.
   */
   ...
   * @throws java.lang.ArithmeticException If <code>n</code> is zero, or <code>x</code> is negative and <code>n</code> is even.
   */
   ...
   * @throws java.lang.ArithmeticException If <code>x</code> or <code>n</code> is zero, or <code>x</code> is negative and <code>n</code> is even.
   */
   ...
   * @return Inverse <code>n</code>:th root of <code>x</code>, that is <code>x<sup>-1/n</sup></code>.
   * @throws java.lang.IllegalArgumentException If <code>targetPrecision <= 0</code>.
   * @throws java.lang.ArithmeticException      If <code>x</code> or <code>n</code> is zero, or <code>x</code> is negative and <code>n</code> is even.
   */
  ...
  public static Apfloat inverseRoot(Apfloat x, long n, long targetPrecision, Apfloat initialGuess)
      throws IllegalArgumentException, ArithmeticException, ApfloatRuntimeException {
    return inverseRoot(x, n, targetPrecision, initialGuess,
        initialGuess == null ? 0 : initialGuess.precision());
  }
   ...
   * @return Inverse <code>n</code>:th root of <code>x</code>, that is <code>x<sup>-1/n</sup></code>.
   * @throws java.lang.IllegalArgumentException If <code>targetPrecision <= 0</code> or <code>initialPrecision <= 0</code>.
   * @throws java.lang.ArithmeticException      If <code>x</code> or <code>n</code> is zero, or <code>x</code> is negative and <code>n</code> is even.
   */
  ...
  public static Apfloat inverseRoot(Apfloat x, long n, long targetPrecision, Apfloat initialGuess,
    ...
    if (x.signum() == 0) {
      ...
      throw new IllegalArgumentException(
          "Target precision " + targetPrecision + " is not positive");
    ...
    if (initialGuess == null || initialPrecision < doublePrecision) {
      ...
      result = new Apfloat(
          (double) result.signum() * Math.pow(Math.abs(result.doubleValue()), -1.0 / (double) n)
              * Math.pow((double) x.radix(), (double) -scaleRem / (double) n), precision,
          x.radix());
   ...
   * @throws java.lang.IllegalArgumentException If <code>precision</code> is less than zero or zero.
   * @throws java.lang.ArithmeticException      If rounding is necessary (result is not exact) and rounding mode is {@link RoundingMode#UNNECESSARY}.
   * @since 1.7.0
   */
  ...
  public static Apfloat scale(Apfloat x, long scale)
    ...
    if ((Math.abs(scale) & 0xC000000000000000L) != 0) {
      ...
      result = (scale >= 0 ? x.multiply(scaler1).multiply(scaler2)
          : x.divide(scaler1).divide(scaler2));
    } else if (x.radix() <= 14) {
  ...
  public static Apfloat fmod(Apfloat x, Apfloat y)
    ...
    } else if (x.precision() <= x.scale() - y
        .scale())                        // We now know that x.scale() >= y.scale()
    {
      return Apfloat.ZERO;                // Degenerate case; not enough precision to make any sense
    } else {
      precision = x.scale() - y.scale()
          + Apfloat.EXTRA_PRECISION;        // Some extra precision to avoid round-off errors
    }
    ...
    precision = Math
        .min(Util.ifFinite(y.precision(), y.precision() + x.scale() - y.scale()), x.precision());
  ...
  private static Apfloat multiplyAddOrSubtract(Apfloat a, Apfloat b, Apfloat c, Apfloat d,
      boolean subtract)
      throws ApfloatRuntimeException {
    long[] precisions;
    Apfloat ab, cd;
  ...
  public static Apfloat agm(Apfloat a, Apfloat b)
    ...
    {
      return a.signum() == b.signum() ? a.precision(Math.min(a.precision(), b.precision()))
          : Apfloat.ZERO;
    }
    ...
    boolean negate = a.signum()
        < 0;                // Thanks to Marko Gaspersic for finding several bugs in issue #12
    if (negate) {
      a = a.negate();
      b = b.negate();
    }
   ...
   * @throws java.lang.NumberFormatException    If the default radix is not valid.
   * @throws java.lang.IllegalArgumentException In case the precision is invalid.
   */
   ...
   * @throws java.lang.NumberFormatException    If the radix is not valid.
   * @throws java.lang.IllegalArgumentException In case the precision is invalid.
   */
  ...
  private static class PiCalculator {
    ...
    private Apfloat p(long n)
      ...
      if (n == 0) {
        ...
        v = sixf.subtract(this.ONE).multiply(this.TWO.multiply(f).subtract(this.ONE))
    ...
    public void r(long n1, long n2, ApfloatHolder T, ApfloatHolder Q, ApfloatHolder P)
      ...
      if (length == 1) {
        ...
        T.setApfloat(
            Q.getApfloat().multiply(LT.getApfloat()).add(LP.getApfloat().multiply(T.getApfloat())));
  ...
  private static Apfloat calculatePi(long precision, Integer radixKey)
    ...
    long neededTerms = (long) ((double) precision * Math.log((double) radix) / 32.65445004177),
        workingPrecision = ApfloatHelper
            .extendPrecision(precision);   // To avoid cumulative round-off errors
   ...
   * @throws java.lang.ArithmeticException If <code>x &lt;= 0</code>.
   */
   ...
   * @throws java.lang.ArithmeticException If <code>x &lt;= 0</code> or <code>b &lt;= 0</code>.
   * @since 1.6
   */
  ...
  public static Apfloat log(Apfloat x, Apfloat b)
    ...
    Apfloat one = new Apfloat(1, Apfloat.INFINITE, x.radix());
    long xPrecision = Util.ifFinite(targetPrecision, targetPrecision + one
        .equalDigits(x)); // If the log() argument is close to 1, the result is less accurate
  ...
  private static Apfloat log(Apfloat x, boolean multiplyByPi)
      throws ArithmeticException, ApfloatRuntimeException {
    if (x.signum() <= 0) {
      throw new ArithmeticException("Logarithm of " + (x.signum() == 0 ? "zero"
          : "negative number; result would be complex"));
    } else if (x.equals(Apfloat.ONE)) {
      return Apfloat.ZERO;
    }
    ...
    Apfloat one = new Apfloat(1, Apfloat.INFINITE, x.radix());
    long finalPrecision = Util.ifFinite(targetPrecision, targetPrecision - one
        .equalDigits(x));     // If the argument is close to 1, the result is less accurate
    if (x.scale() > 1) {
      double logScale = Math.log((double) x.scale() - 1) / Math.log((double) x.radix());
      logScale += Math.ulp(logScale);
      finalPrecision = Util.ifFinite(finalPrecision, finalPrecision
          + (long) logScale);           // If the argument is very big, the result is more accurate
    }
    ...
    if (originalScale == 0) {
      ...
      Apfloat logRadix = ApfloatHelper
          .extendPrecision(logRadix(targetPrecision, x.radix(), multiplyByPi));
      radixPower = new Apfloat(originalScale, Apfloat.INFINITE, x.radix()).multiply(logRadix);
    }

    return ApfloatHelper.extendPrecision(rawLog(x, multiplyByPi)).add(radixPower)
   ...
   * @throws java.lang.NumberFormatException If the radix is invalid.
   */
  ...
  private static Apfloat logRadix(long precision, int radix, boolean multiplyByPi)
    ...
    // - doesn't block getting it for other radixes
    synchronized (radixKey) {
      Map<Integer, Apfloat> cache = (multiplyByPi ? ApfloatMath.radixLogPi
          : ApfloatMath.radixLog);     // Which cache to use, the one multiplied by pi or not
  ...
  public static Apfloat exp(Apfloat x)
    ...
    if (targetPrecision == Apfloat.INFINITE) {
      throw new InfiniteExpansionException("Cannot calculate exponent to infinite precision");
        ...
        new Apfloat((double) Long.MAX_VALUE * Math.log((double) radix), doublePrecision, radix))
        >= 0) {
      throw new OverflowException("Overflow");
    } else if (x.compareTo(
        new Apfloat((double) Long.MIN_VALUE * Math.log((double) radix), doublePrecision, radix))
        <= 0) {
      // Underflow
    ...
    if (x.scale() < -doublePrecision / 2) {
      ...
      result = new Apfloat(Math.pow((double) radix, fractionalPart.doubleValue()), doublePrecision,
          radix);
   ...
   * @throws java.lang.ArithmeticException If both <code>x</code> and <code>y</code> are zero, or <code>x</code> is negative.
   */
  ...
  public static Apfloat pow(Apfloat x, Apfloat y)
    ...
    Apfloat one = new Apfloat(1, Apfloat.INFINITE, x.radix());
    targetPrecision = Util.ifFinite(targetPrecision, targetPrecision + one
        .equalDigits(x)); // If the log() argument is close to 1, the result is less accurate
   ...
   * @throws java.lang.ArithmeticException If <code>x &lt; 1</code>.
   */
   ...
   * @throws java.lang.ArithmeticException If <code>abs(x) &gt;= 1</code>.
   */
  ...
  public static Apfloat asin(Apfloat x)
    ...
    return ApcomplexMath.log(sqrt(one.subtract(x.multiply(x))).subtract(i.multiply(x))).imag()
        .negate();
  }
   ...
   * @throws java.lang.ArithmeticException If <code>y</code> and <code>x</code> are both zero.
   */
   ...
   * @throws java.lang.ArithmeticException If <code>x</code> is &pi;/2 + n &pi; where n is an integer.
   */
   ...
   * @throws java.lang.ArithmeticException If <code>x</code> is less than -1/e.
   * @since 1.8.0
   */
  ...
  public static Apfloat toDegrees(Apfloat x)
      throws ApfloatRuntimeException {
    return x.signum() == 0 ? x : x.multiply(new Apfloat(180, Apfloat.INFINITE, x.radix())).divide(
        pi(x.precision(),
            x.radix()));    // Thanks to Marko Gaspersic for finding the bug in issue #11
  }
  ...
  public static Apfloat toRadians(Apfloat x)
      throws ApfloatRuntimeException {
    return x.signum() == 0 ? x : x.divide(new Apfloat(180, Apfloat.INFINITE, x.radix())).multiply(
        pi(x.precision(),
            x.radix()));    // Thanks to Marko Gaspersic for finding the bug in issue #11
  }
  ...
  public static Apfloat sum(Apfloat... x)
    ...
    if (x.length >= 1000) {
      ...
      ApfloatContext ctx = ApfloatContext.getContext();
      long maxSize = (long) (ctx.getMemoryThreshold() * 5.0 / Math
          .log((double) ctx.getDefaultRadix()));
   ...
   * @throws java.lang.NumberFormatException    If the default radix is not valid.
   * @throws java.lang.IllegalArgumentException In case the number of specified digits is invalid.
   * @since 1.9.0
   */
  ...
  private static Apfloat lastIterationExtendPrecision(int iterations, int precisingIteration,
      Apfloat x)
      throws ApfloatRuntimeException {
  ...
  private static Integer getRadixKey(Map<Integer, Integer> radixKeys, int radix) {
    ...
    Integer value = new Integer(
        radix); // Use new Integer since we synchronize on it; Integer.valueOf() could be shared instance
  ...
  static Apfloat factorial(long n, long precision, int radix)
    ...
    return factorialProduct.multiply(pow(new Apfloat(2, precision, radix), exponentOfTwo))
