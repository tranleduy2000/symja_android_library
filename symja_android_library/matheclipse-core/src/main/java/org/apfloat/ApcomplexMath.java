...
public class ApcomplexMath {
  ...
  public static Apcomplex inverseRoot(Apcomplex z, long n, long k)
    ...
    if (z.imag().signum() == 0 && z.real().signum() > 0 && k == 0) {
      ...
      return inverseRootAbs(inverseRootAbs(z, -n, k), 1,
          0);      // Also correctly handles 0x8000000000000000L
    }
  ...
  private static Apcomplex inverseRootAbs(Apcomplex z, long n, long k)
    ...
    if (z.imag().signum() == 0 ||
        (scaleDiff > doublePrecision / 2 || scaleDiff < 0)
            && realScale > imagScale)        // Detect overflow
    {
      ...
      tmpReal = ApfloatMath
          .scale(tmpReal, -tmpReal.scale());     // Allow exponents in excess of doubles'
      ...
      result = result.subtract(result.multiply(tweak));               // Must not be real
        ...
        (scaleDiff > doublePrecision / 2 || scaleDiff < 0)
            && imagScale > realScale)        // Detect overflow
    {
      ...
      tmpImag = ApfloatMath
          .scale(tmpImag, -tmpImag.scale());     // Allow exponents in exess of doubles'
      ...
      magnitude =
          Math.pow(doubleReal * doubleReal + doubleImag * doubleImag, -1.0 / (2.0 * doubleN))
              * scaleRemFactor;
  ...
  public static Apcomplex[] allRoots(Apcomplex z, int n)
    ...
    long precision = z.precision();
    z = ApfloatHelper
        .extendPrecision(z);                   // Big roots will accumulate round-off errors
  ...
  public static Apcomplex log(Apcomplex z)
    ...
    if (z.real().signum() < 0) {
      Apfloat pi = ApfloatHelper.extendPrecision(ApfloatMath.pi(targetPrecision, z.radix()),
          z.radix() <= 3 ? 1 : 0);     // pi may have 1 digit more than pi/2
    ...
    if (originalScale == 0) {
      ...
      Apfloat logRadix = ApfloatHelper
          .extendPrecision(ApfloatMath.logRadix(targetPrecision, z.radix()));
    ...
    long finalRealPrecision = Math.max(targetPrecision - one.equalDigits(x), 1),
        finalImagPrecision = Math
            .max(targetPrecision - 1 + result.imag().scale(), 1);     // Scale of pi/2 is always 1
  ...
  public static Apcomplex log(Apcomplex z, Apcomplex w)
    ...
    if (z.real().signum() >= 0 && z.imag().signum() == 0) {
      ...
      targetPrecision = Util.ifFinite(targetPrecision, targetPrecision + one
          .equalDigits(y)); // If the log() argument is close to 1, the result is less accurate
  ...
  private static Apcomplex rawLog(Apcomplex z)
    ...
    Apfloat pi = ApfloatHelper.extendPrecision(ApfloatMath.pi(targetPrecision, z.radix()));
    Apcomplex log = pi.multiply(agmez.subtract(agme))
        .divide(new Apfloat(2, Apfloat.INFINITE, z.radix()).multiply(agme).multiply(agmez));
  ...
  public static Apcomplex exp(Apcomplex z)
    ...
    if (z.imag().scale() > 0) {
      ...
      Apfloat pi = ApfloatMath
          .pi(piPrecision, radix),    // This is precalculated for initial check only
          twoPi = pi.add(pi),
    ...
    if (z.real().signum() == 0) {
      ...
      resultReal = new Apfloat(Math.pow((double) radix, fractionalPart.doubleValue()),
  ...
  public static Apcomplex gamma(Apcomplex z)
    ...
    long workingPrecision = ApfloatHelper.extendPrecision(precision, (long) (precision * 0.5)
        + Apfloat.EXTRA_PRECISION); // increase intermediate precision - ck are large and alternating in sign, lots of precision loss
    ...
    Apcomplex result = ApcomplexMath.pow(z.add(a), z.add(half))
        .multiply(ApcomplexMath.exp(z.negate().subtract(a))).multiply(sum);
    double normalizedScale = result.scale() * Math.log(radix);
    if (normalizedScale > 0 && z.real().scale() > 0) {
      precision = precision - (long) (1.01 * Math.log(normalizedScale) / Math
          .log(radix)); // Very large results have a reduced precision
      if (precision <= 0) {
        throw new LossOfPrecisionException("Complete loss of accurate digits");
      }
    } else if (normalizedScale < 0) {
      precision = precision - (long) (1.148 * Math.log(-normalizedScale) / Math
          .log(radix)); // Very small results also have a reduced precision
      if (precision <= 0) {
        throw new LossOfPrecisionException("Complete loss of accurate digits");
      }
    }
  ...
  private static Apcomplex lastIterationExtendPrecision(int iterations, int precisingIteration,
      Apcomplex z) {
