...
//	private static final Logger LOG = Logger.getLogger(Lehman_Analyzer2.class);
	...
	private static final int N_MOD = 8;
	private static final int k_MOD = 6;
	private static final int a_MOD = 8;
		...
		for (int k=1; k <= cbrt; k++) {
			...
			for (long a = sqrt4kN; a <= limit; a++) {
				...
				if (b*b == test) {
					...
					if (gcd>1 && gcd<N) {
						aValues[(int)((k+N)%N_MOD)][k%k_MOD].add((int) (a%a_MOD));
						return gcd;
					}
				}
			}
...
//					LOG.info("Successful a-values %" + a_MOD + " for (k+N)%" + N_MOD + "==" + NMod + ", k%" + k_MOD + "==" + kMod + " : " + totalCount + " (" + aCounts + ")");
